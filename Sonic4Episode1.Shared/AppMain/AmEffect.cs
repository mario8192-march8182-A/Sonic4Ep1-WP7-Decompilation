...
public partial class AppMain
{
    ...
    public static void amEffectLerpColor(out AMS_RGBA8888 pCO, ref AMS_RGBA8888 pC1, ref AMS_RGBA8888 pC2, float rate)
    {
        int num = (int) (rate * 256f);
        pCO = default(AMS_RGBA8888);
    ...
    public static void amEffectSystemInit()
    {
        ...
        AppMain._am_ecb_buf = AppMain.New<AMS_AME_ECB>(128);
        AppMain._am_ecb_ref = new AMS_AME_ECB[128];
        for (int i = 0; i < 128; i++)
        {
        ...
        AppMain._am_entry_buf = new AMS_AME_ENTRY[512];
        AppMain._am_entry_ref = new AMS_AME_ENTRY[512];
        for (int j = 0; j < 512; j++)
        {
            AppMain._am_entry_buf[j] = new AMS_AME_ENTRY();
            AppMain._am_entry_ref[j] = AppMain._am_entry_buf[j];
        }

        AppMain._am_runtime_alloc = 0;
        AppMain._am_runtime_free = 0;
        AppMain._am_runtime_buf = new AMS_AME_RUNTIME[512];
        AppMain._am_runtime_ref = new AMS_AME_RUNTIME[512];
        for (int k = 0; k < 512; k++)
        {
            AppMain._am_runtime_buf[k] = new AMS_AME_RUNTIME();
            AppMain._am_runtime_ref[k] = AppMain._am_runtime_buf[k];
        }

        AppMain._am_work_alloc = 0;
        AppMain._am_work_free = 0;
        AppMain._am_work_buf = new AMS_AME_RUNTIME_WORK[1024];
        AppMain._am_work_ref = new AMS_AME_RUNTIME_WORK[1024];
        for (int l = 0; l < 1024; l++)
        {
            AppMain._am_work_buf[l] = new AMS_AME_RUNTIME_WORK();
    ...
    {
        for (AMS_AME_ECB ams_AME_ECB = (AMS_AME_ECB) AppMain._am_ecb_head.next;
            ams_AME_ECB != AppMain._am_ecb_tail;
            ams_AME_ECB = (AMS_AME_ECB) ams_AME_ECB.next)
        {
    ...
    public static void amEffectSetObject(AMS_AME_ECB ecb, AppMain.NNS_OBJECT object_, int state)
    {
        ecb.pObj = object_;
    ...
    {
        AppMain.mppAssertNotImpl();
        AMS_AME_NODE ams_AME_NODE = null;
        if ((int) node.id == id)
        {
            return node;
        }
    ...
    private AMS_AME_ECB amEffectCreate(AMS_AME_NODE node, int attribute, int priority)
    {
        AppMain.mppAssertNotImpl();
        return null;
    }
    ...
    private static AMS_AME_ECB _amEffectCreate(AMS_AME_HEADER header, int attribute, int priority)
    {
        AMS_AME_ECB ams_AME_ECB = AppMain._am_ecb_ref[AppMain._am_ecb_alloc];
        ...
        AMS_AME_ECB ams_AME_ECB2 = (AMS_AME_ECB) AppMain._am_ecb_head.next;
        while (ams_AME_ECB2 != AppMain._am_ecb_tail && ams_AME_ECB2.priority <= priority)
        {
            ams_AME_ECB2 = (AMS_AME_ECB) ams_AME_ECB2.next;
        }
        ...
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = new AMS_AME_CREATE_PARAM();
        NNS_VECTOR4D amEffectCreate_vec = AppMain._amEffectCreate_vec;
        AMS_AME_NODE ams_AME_NODE = header.node[0];
        ...
        while (ams_AME_NODE != null)
        {
            ...
            int num = AmAme.SUPER_CLASS_ID(ams_AME_NODE);
            if (num != 256)
            {
                if (num == 512)
                {
                    AMS_AME_RUNTIME ams_AME_RUNTIME = AppMain._amCreateRuntimeParticle(ams_AME_CREATE_PARAM);
                    ams_AME_RUNTIME.state |= 8192;
                }
            }
            else
            {
                AMS_AME_RUNTIME ams_AME_RUNTIME = AppMain._amCreateRuntimeEmitter(ams_AME_CREATE_PARAM);
                ams_AME_RUNTIME.state |= 8192;
            }
        ...
        switch (flag)
        {
            case 0:
                for (AMS_AME_ECB ams_AME_ECB = (AMS_AME_ECB) AppMain._am_ecb_head.next;
                    ...
                    ams_AME_ECB = (AMS_AME_ECB) ams_AME_ECB.next)
                {
                    if (((long) ams_AME_ECB.attribute & (long) ((ulong) num)) != 0L &&
                        ((long) ams_AME_ECB.attribute & (long) ((ulong) num2)) != 0L)
                    {
            ...
            case 1:
                for (AMS_AME_ECB ams_AME_ECB = (AMS_AME_ECB) AppMain._am_ecb_head.next;
                    ...
                    ams_AME_ECB = (AMS_AME_ECB) ams_AME_ECB.next)
                {
                    if ((((long) ams_AME_ECB.attribute & (long) ((ulong) num)) |
                         ((long) ams_AME_ECB.attribute & (long) ((ulong) num2))) == (long) attr)
                    {
                        AppMain.amEffectDelete(ams_AME_ECB);
                    }
                }
    ...
    {
        for (AMS_AME_ENTRY ams_AME_ENTRY = ecb.entry_head;
            ams_AME_ENTRY != null;
            ams_AME_ENTRY = (AMS_AME_ENTRY) ams_AME_ENTRY.next)
        {
            AMS_AME_RUNTIME runtime = ams_AME_ENTRY.runtime;
            AMS_AME_NODE node = runtime.node;
            if ((AmAme.NODE_TYPE(node) & 65280) == 256 && (runtime.state & 32768) == 0)
            {
                ...
                AMS_AME_LIST child_tail = runtime.child_tail;
                while (next != child_tail)
                {
                    AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) next;
                ...
                if (runtime.parent_runtime != null)
                {
                    ...
                    AMS_AME_RUNTIME parent_runtime = runtime.parent_runtime;
    ...
    private void amEffectKillGroup(int attr, int flag)
    {
        ...
        switch (flag)
        {
            ...
            case 1:
                ...
                {
                    if (((long) ams_AME_ECB.attribute & (long) ((ulong) num)) != 0L &&
                        ((long) ams_AME_ECB.attribute & (long) ((ulong) num2)) == (long) ((ulong) num2))
                    {
                        AppMain.amEffectKill(ams_AME_ECB);
                    }
                }
    ...
    public static void amEffectUpdate(AMS_AME_ECB ecb)
    {
            ...
            else
            {
                ...
                if (num2 != 256)
                {
                    if (num2 == 512)
                    {
                        ...
                        {
                            AMS_AME_RUNTIME_WORK ams_AME_RUNTIME_WORK = (AMS_AME_RUNTIME_WORK) runtime.work_head.next;
                            AMS_AME_RUNTIME_WORK ams_AME_RUNTIME_WORK2 = (AMS_AME_RUNTIME_WORK) runtime.work_tail;
                            while (ams_AME_RUNTIME_WORK != ams_AME_RUNTIME_WORK2)
                            {
                                AMS_AME_RUNTIME_WORK ams_AME_RUNTIME_WORK3 = ams_AME_RUNTIME_WORK;
                                ams_AME_RUNTIME_WORK = (AMS_AME_RUNTIME_WORK) ams_AME_RUNTIME_WORK.next;
                                ...
                                if (ams_AME_RUNTIME_WORK3.time > 0f)
                                {
                                    ...
                                    AMS_AME_RUNTIME ams_AME_RUNTIME = runtime;
                                    ams_AME_RUNTIME.work_num -= 1;
                                    AMS_AME_RUNTIME ams_AME_RUNTIME2 = runtime;
                                    ams_AME_RUNTIME2.active_num += 1;
                                }
                            }
                        }

                        AmeDelegateFunc ameDelegateFunc = AppMain._am_particle_func[((num & 255) << 2) + 1];
                        ameDelegateFunc(runtime);
                        AMS_AME_RUNTIME_WORK ams_AME_RUNTIME_WORK4 = (AMS_AME_RUNTIME_WORK) runtime.active_head.next;
                        AMS_AME_LIST active_tail = runtime.active_tail;
                        for (AMS_AME_LIST next = runtime.active_head.next;
                            next != active_tail;
                            next = ams_AME_RUNTIME_WORK4.next)
                        {
                            ams_AME_RUNTIME_WORK4 = (AMS_AME_RUNTIME_WORK) next;
                            for (AMS_AME_NODE ams_AME_NODE = node.child;
                                ams_AME_NODE != null;
                                ams_AME_NODE = ams_AME_NODE.sibling)
                            {
                                if (AmAme.IS_FIELD(ams_AME_NODE))
                                {
                                    AmeFieldFunc ameFieldFunc =
                                        AppMain._am_field_func[AmAme.NODE_TYPE(ams_AME_NODE) & 255];
                ...
                {
                    AmeDelegateFunc ameDelegateFunc2 = AppMain._am_emitter_func[((num & 255) << 2) + 1];
                    if (runtime.work != null)
                    {
                        if (ameDelegateFunc2(runtime) != 0)
                        {
                            runtime.state |= 32768;
                            AMS_AME_LIST next2 = runtime.child_head.next;
                            AMS_AME_LIST child_tail = runtime.child_tail;
                            while (next2 != child_tail)
                            {
                                AMS_AME_RUNTIME ams_AME_RUNTIME3 = (AMS_AME_RUNTIME) next2;
                        ...
                        {
                            for (AMS_AME_NODE ams_AME_NODE2 = node.child;
                                ams_AME_NODE2 != null;
                                ams_AME_NODE2 = ams_AME_NODE2.sibling)
                            {
                                if (AmAme.IS_FIELD(ams_AME_NODE2))
                                {
                                    AmeFieldFunc ameFieldFunc2 =
                                        AppMain._am_field_func[AmAme.NODE_TYPE(ams_AME_NODE2) & 255];
        ...
        for (AMS_AME_ENTRY ams_AME_ENTRY = ecb.entry_head;
            ...
            ams_AME_ENTRY = (AMS_AME_ENTRY) ams_AME_ENTRY.next)
        {
            AMS_AME_RUNTIME runtime2 = ams_AME_ENTRY.runtime;
    ...
    public static void amEffectDraw(AMS_AME_ECB ecb, AppMain.NNS_TEXLIST texlist, uint state)
    {
        ecb.drawState = state;
            ...
            ams_AME_ENTRY = (AMS_AME_ENTRY) ams_AME_ENTRY.next)
        {
            ...
            int num = AmAme.SUPER_CLASS_ID(runtime.node);
            if (num == 512 && runtime.active_num != 0)
            {
                AmeDelegateFunc ameDelegateFunc =
                    AppMain._am_particle_func[(AmAme.CLASS_ID(runtime.node) << 2) + 2];
                ameDelegateFunc(runtime);
            }
        }
    }
        ...
        switch (flag)
        {
            case 0:
                for (AMS_AME_ECB ams_AME_ECB = (AMS_AME_ECB) AppMain._am_ecb_head.next;
                    ...
                    ams_AME_ECB = (AMS_AME_ECB) ams_AME_ECB.next)
                {
                    if (0L != ((long) ams_AME_ECB.attribute & (long) ((ulong) num)) &&
                        0L != ((long) ams_AME_ECB.attribute & (long) ((ulong) num2)))
                    {
    ...
    public static void amEffectSetTranslate(AMS_AME_ECB ecb, ref SNNS_VECTOR4D translate)
    {
            ...
            ams_AME_ENTRY = (AMS_AME_ENTRY) ams_AME_ENTRY.next)
        {
            ...
            if ((runtime.state & 8192) != 0 && (runtime.node.flag & 67108864U) == 0U)
            {
                if (runtime.work != null)
                {
                    AppMain.amVectorAdd(runtime.work.position, ((AMS_AME_NODE_TR_ROT) runtime.node).translate,
                        ref translate);
                }
                ...
                if (runtime.work_num + runtime.active_num != 0)
                {
                    ...
                    while (ams_AME_RUNTIME_WORK != ams_AME_RUNTIME_WORK2)
                    {
                        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK.position,
                            ((AMS_AME_NODE_TR_ROT) runtime.node).translate, ref translate);
                        ams_AME_RUNTIME_WORK = (AMS_AME_RUNTIME_WORK) ams_AME_RUNTIME_WORK.next;
                    }

                    ams_AME_RUNTIME_WORK = (AMS_AME_RUNTIME_WORK) runtime.active_head.next;
                    ams_AME_RUNTIME_WORK2 = (AMS_AME_RUNTIME_WORK) runtime.active_tail;
                    while (ams_AME_RUNTIME_WORK != ams_AME_RUNTIME_WORK2)
                    {
                        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK.position,
                            ((AMS_AME_NODE_TR_ROT) runtime.node).translate, ref translate);
            ...
            ams_AME_ENTRY = (AMS_AME_ENTRY) ams_AME_ENTRY.next)
        {
            ...
            if ((runtime.state & 8192) != 0 && (runtime.node.flag & 67108864U) == 0U)
            {
                ...
                if (runtime.work_num + runtime.active_num != 0)
                {
                    ...
                    {
                        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK.position,
                            ((AMS_AME_NODE_TR_ROT) runtime.node).translate, translate);
                        ams_AME_RUNTIME_WORK = (AMS_AME_RUNTIME_WORK) ams_AME_RUNTIME_WORK.next;
                    }
    ...
    private void amEffectTranslate(AMS_AME_ECB ecb, NNS_VECTOR4D translate)
    {
            ...
            ams_AME_ENTRY = (AMS_AME_ENTRY) ams_AME_ENTRY.next)
        {
            ...
            if ((runtime.state & 8192) != 0 && (runtime.node.flag & 67108864U) == 0U)
            {
                ...
                if (runtime.work_num + runtime.active_num != 0)
                {
                    AMS_AME_LIST next = runtime.work_head.next;
                    AMS_AME_LIST ams_AME_LIST = runtime.work_tail;
    ...
    public static void amEffectSetRotate(AMS_AME_ECB ecb, ref NNS_QUATERNION q, int offset)
    {
        ecb.rotate = q;
        for (AMS_AME_ENTRY ams_AME_ENTRY = ecb.entry_head;
            ams_AME_ENTRY != null;
            ams_AME_ENTRY = (AMS_AME_ENTRY) ams_AME_ENTRY.next)
        {
            ...
            if ((runtime.state & 8192) != 0 && (runtime.node.flag & 134217728U) == 0U)
            {
                if (runtime.work != null)
                {
                    if (offset != 0)
                    {
                        AppMain.amQuatMulti(ref runtime.work.rotate[0], ref ((AMS_AME_NODE_OMNI) runtime.node).rotate,
                            ref q);
                    }
                    else
                    {
                ...
                if (runtime.work_num + runtime.active_num != 0)
                {
                    ...
                    while (ams_AME_RUNTIME_WORK != ams_AME_RUNTIME_WORK2)
                    {
                        if (offset != 0)
                        {
                            AppMain.amQuatMulti(ref ams_AME_RUNTIME_WORK.rotate[0],
                                ref ((AMS_AME_NODE_OMNI) runtime.node).rotate, ref q);
                        }
                        else
                        {
    ...
    public static int _amEffectFrustumCulling(NNS_VECTOR4D pPos, AMS_FRUSTUM pFrustum, AMS_AME_BOUNDING pBounding)
    {
        return 1;
    }
    ...
    public static void _amEffectFinalize(AMS_AME_ECB ecb)
    {
            ...
            ams_AME_ENTRY = (AMS_AME_ENTRY) ams_AME_ENTRY.next)
        {
            AMS_AME_RUNTIME runtime = ams_AME_ENTRY.runtime;
            AMS_AME_NODE node = runtime.node;
            if (AmAme.SUPER_CLASS_ID(node) == 256 && runtime.parent_runtime != null)
            {
                AppMain.amEffectDisconnectLink(runtime);
                AMS_AME_RUNTIME parent_runtime = runtime.parent_runtime;
                parent_runtime.work_num -= 1;
    ...
    public static AMS_AME_RUNTIME _amCreateRuntimeEmitter(AMS_AME_CREATE_PARAM param)
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = AppMain._amAllocRuntime();
        ...
        AmeDelegateFunc ameDelegateFunc = AppMain._am_emitter_func[(AmAme.NODE_TYPE(param.node) & 255) << 2];
        ameDelegateFunc(param);
        for (AMS_AME_NODE ams_AME_NODE = param.node.child; ams_AME_NODE != null; ams_AME_NODE = ams_AME_NODE.sibling)
        {
            if (!AmAme.IS_FIELD(ams_AME_NODE))
            {
                AMS_AME_RUNTIME list = AppMain._amCreateRuntimeGroup(param.ecb, ams_AME_NODE);
    ...
    private static AMS_AME_RUNTIME _amCreateRuntimeParticle(AMS_AME_CREATE_PARAM param)
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = AppMain._amAllocRuntime();
        ams_AME_RUNTIME.ecb = param.ecb;
        ams_AME_RUNTIME.node = param.node;
        ...
        for (AMS_AME_NODE ams_AME_NODE = param.node.child; ams_AME_NODE != null; ams_AME_NODE = ams_AME_NODE.sibling)
        {
            if (AmAme.IS_PARTICLE(ams_AME_NODE))
            {
    ...
    public static AMS_AME_RUNTIME _amCreateRuntimeGroup(AMS_AME_ECB ecb, AMS_AME_NODE node)
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = AppMain._amAllocRuntime();
        ...
        ams_AME_RUNTIME.active_head.next = ams_AME_RUNTIME.active_tail;
        ams_AME_RUNTIME.active_tail.prev = ams_AME_RUNTIME.active_head;
        for (AMS_AME_NODE ams_AME_NODE = node.child; ams_AME_NODE != null; ams_AME_NODE = ams_AME_NODE.sibling)
        {
            if (AmAme.IS_PARTICLE(ams_AME_NODE))
            {
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = AppMain._amCreateRuntimeEmitter(param);
        ams_AME_RUNTIME.parent_runtime = param.runtime;
        AppMain._amConnectLinkToTail(ams_AME_RUNTIME.work_tail, ams_AME_RUNTIME);
        AMS_AME_RUNTIME ams_AME_RUNTIME2 = ams_AME_RUNTIME;
    ...
    {
        AMS_AME_RUNTIME_WORK_MODEL work = (AMS_AME_RUNTIME_WORK_MODEL) AppMain._amAllocRuntimeWork();
        param.work = work;
        AmeDelegateFunc ameDelegateFunc = AppMain._am_particle_func[(AmAme.NODE_TYPE(param.node) & 255) << 2];
        ameDelegateFunc(param);
        if (work.time < 0f)
        {
            AppMain._amConnectLinkToTail(param.runtime.work_tail, (AMS_AME_LIST) work);
            AMS_AME_RUNTIME runtime = param.runtime;
            runtime.work_num += 1;
            return;
        }

        AppMain._amConnectLinkToTail(param.runtime.active_tail, (AMS_AME_LIST) work);
        AMS_AME_RUNTIME runtime2 = param.runtime;
        runtime2.active_num += 1;
    }

    // Token: 0x06001A83 RID: 6787 RVA: 0x000ECEDC File Offset: 0x000EB0DC
    public static void _amCreateSpawnParticle(AMS_AME_RUNTIME runtime, AMS_AME_RUNTIME_WORK work)
    {
        NNS_VECTOR4D nns_VECTOR4D = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        AppMain.amVectorInit(nns_VECTOR4D);
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = AppMain.GlobalPool<AMS_AME_CREATE_PARAM>.Alloc();
        ...
        if (AmAme.NODE_TYPE(runtime.node) == AmAme.NODE_TYPE(ams_AME_CREATE_PARAM.node))
        {
            AMS_AME_NODE node = runtime.node;
            AMS_AME_NODE node2 = ams_AME_CREATE_PARAM.node;
            switch (AmAme.NODE_TYPE(ams_AME_CREATE_PARAM.node))
            {
                case 512:
                {
                    AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE ams_AME_RUNTIME_WORK_SIMPLE_SPRITE =
                        (AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE) work;
                    AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2 =
                        (AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE) ams_AME_CREATE_PARAM.work;
                    if ((node.flag & node2.flag & 131072U) != 0U &&
                        ((ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.flag ^ ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.flag) & 8U) !=
                        0U)
                    {
                        ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.flag ^= 8U;
                        ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.set_st(ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.st.z,
                            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.st.y, ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.st.x,
                            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.st.w);
                    }

                    if ((node.flag & node2.flag & 262144U) != 0U &&
                        ((ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.flag ^ ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.flag) & 16U) !=
                        0U)
                    {
                        ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.flag ^= 16U;
                        ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.set_st(ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.st.x,
                            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.st.w, ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.st.z,
                            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE2.st.y);
                    }
                ...
                case 513:
                {
                    AMS_AME_RUNTIME_WORK_SPRITE ams_AME_RUNTIME_WORK_SPRITE = (AMS_AME_RUNTIME_WORK_SPRITE) work;
                    AMS_AME_RUNTIME_WORK_SPRITE ams_AME_RUNTIME_WORK_SPRITE2 =
                        (AMS_AME_RUNTIME_WORK_SPRITE) ams_AME_CREATE_PARAM.work;
                    ...
                    if ((node.flag & node2.flag & 131072U) != 0U &&
                        ((ams_AME_RUNTIME_WORK_SPRITE.flag ^ ams_AME_RUNTIME_WORK_SPRITE2.flag) & 8U) != 0U)
                    {
                        ams_AME_RUNTIME_WORK_SPRITE2.flag ^= 8U;
                        ams_AME_RUNTIME_WORK_SPRITE2.set_st(ams_AME_RUNTIME_WORK_SPRITE2.st.z,
                            ams_AME_RUNTIME_WORK_SPRITE2.st.y, ams_AME_RUNTIME_WORK_SPRITE2.st.x,
                            ams_AME_RUNTIME_WORK_SPRITE2.st.w);
                    }

                    if ((node.flag & node2.flag & 262144U) != 0U &&
                        ((ams_AME_RUNTIME_WORK_SPRITE.flag ^ ams_AME_RUNTIME_WORK_SPRITE2.flag) & 16U) != 0U)
                    {
                        ams_AME_RUNTIME_WORK_SPRITE2.flag ^= 16U;
                        ams_AME_RUNTIME_WORK_SPRITE2.set_st(ams_AME_RUNTIME_WORK_SPRITE2.st.x,
                            ams_AME_RUNTIME_WORK_SPRITE2.st.w, ams_AME_RUNTIME_WORK_SPRITE2.st.z,
                            ams_AME_RUNTIME_WORK_SPRITE2.st.y);
                    }
                ...
                case 514:
                {
                    AMS_AME_RUNTIME_WORK_LINE ams_AME_RUNTIME_WORK_LINE = (AMS_AME_RUNTIME_WORK_LINE) work;
                    AMS_AME_RUNTIME_WORK_LINE ams_AME_RUNTIME_WORK_LINE2 =
                        (AMS_AME_RUNTIME_WORK_LINE) ams_AME_CREATE_PARAM.work;
                    if ((node.flag & node2.flag & 131072U) != 0U &&
                        ((ams_AME_RUNTIME_WORK_LINE.flag ^ ams_AME_RUNTIME_WORK_LINE2.flag) & 8U) != 0U)
                    {
                        ams_AME_RUNTIME_WORK_LINE2.flag ^= 8U;
                        ams_AME_RUNTIME_WORK_LINE2.set_st(ams_AME_RUNTIME_WORK_LINE2.st.z,
                            ams_AME_RUNTIME_WORK_LINE2.st.y, ams_AME_RUNTIME_WORK_LINE2.st.x,
                            ams_AME_RUNTIME_WORK_LINE2.st.w);
                    }

                    if ((node.flag & node2.flag & 262144U) != 0U &&
                        ((ams_AME_RUNTIME_WORK_LINE.flag ^ ams_AME_RUNTIME_WORK_LINE2.flag) & 16U) != 0U)
                    {
                        ams_AME_RUNTIME_WORK_LINE2.flag ^= 16U;
                        ams_AME_RUNTIME_WORK_LINE2.set_st(ams_AME_RUNTIME_WORK_LINE2.st.x,
                            ams_AME_RUNTIME_WORK_LINE2.st.w, ams_AME_RUNTIME_WORK_LINE2.st.z,
                            ams_AME_RUNTIME_WORK_LINE2.st.y);
                    }
                ...
                case 515:
                {
                    AMS_AME_RUNTIME_WORK_PLANE ams_AME_RUNTIME_WORK_PLANE = (AMS_AME_RUNTIME_WORK_PLANE) work;
                    AMS_AME_RUNTIME_WORK_PLANE ams_AME_RUNTIME_WORK_PLANE2 =
                        (AMS_AME_RUNTIME_WORK_PLANE) ams_AME_CREATE_PARAM.work;
                    if ((node.flag & node2.flag & 131072U) != 0U &&
                        ((ams_AME_RUNTIME_WORK_PLANE.flag ^ ams_AME_RUNTIME_WORK_PLANE2.flag) & 8U) != 0U)
                    {
                        ams_AME_RUNTIME_WORK_PLANE2.flag ^= 8U;
                        ams_AME_RUNTIME_WORK_PLANE2.set_st(ams_AME_RUNTIME_WORK_PLANE2.st.z,
                            ams_AME_RUNTIME_WORK_PLANE2.st.y, ams_AME_RUNTIME_WORK_PLANE2.st.x,
                            ams_AME_RUNTIME_WORK_PLANE2.st.w);
                    }

                    if ((node.flag & node2.flag & 262144U) != 0U &&
                        ((ams_AME_RUNTIME_WORK_PLANE.flag ^ ams_AME_RUNTIME_WORK_PLANE2.flag) & 16U) != 0U)
                    {
                        ams_AME_RUNTIME_WORK_PLANE2.flag ^= 16U;
                        ams_AME_RUNTIME_WORK_PLANE2.set_st(ams_AME_RUNTIME_WORK_PLANE2.st.x,
                            ams_AME_RUNTIME_WORK_PLANE2.st.w, ams_AME_RUNTIME_WORK_PLANE2.st.z,
                            ams_AME_RUNTIME_WORK_PLANE2.st.y);
                    }
        ...
        AppMain.GlobalPool<AMS_AME_CREATE_PARAM>.Release(ams_AME_CREATE_PARAM);
        AppMain.GlobalPool<NNS_VECTOR4D>.Release(nns_VECTOR4D);
    }
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = AppMain._am_runtime_ref[AppMain._am_runtime_alloc];
    ...
    public static void _amFreeRuntime(AMS_AME_RUNTIME runtime)
    {
        if (runtime.work != null)
        {
            AMS_AME_RUNTIME_WORK work = runtime.work;
    ...
    {
        AMS_AME_RUNTIME_WORK ams_AME_RUNTIME_WORK = AppMain._am_work_ref[AppMain._am_work_alloc];
    ...
    {
        AMS_AME_ENTRY ams_AME_ENTRY = AppMain._am_entry_ref[AppMain._am_entry_alloc];
    ...
    private static int _amEffectSetDrawMode(AMS_AME_RUNTIME runtime, AppMain.AMS_PARAM_DRAW_PRIMITIVE param,
        int NodeBlend)
    {
        AMS_AME_NODE node = runtime.node;
        int num = 0;
    ...
    private static uint _amEffectSetMaterial(AMS_AME_RUNTIME runtime, ref int blend, int NodeBlend)
    {
        AMS_AME_NODE node = runtime.node;
        uint num = 0U;
        if ((node.flag & 1U) != 0U)
    ...
    {
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = (AMS_AME_CREATE_PARAM) p;
        AMS_AME_NODE_OMNI ams_AME_NODE_OMNI = (AMS_AME_NODE_OMNI) ams_AME_CREATE_PARAM.node;
        AMS_AME_RUNTIME_WORK_OMNI ams_AME_RUNTIME_WORK_OMNI = (AMS_AME_RUNTIME_WORK_OMNI) ams_AME_CREATE_PARAM.work;
        ams_AME_RUNTIME_WORK_OMNI.time = -ams_AME_NODE_OMNI.start_time;
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_OMNI.position, ams_AME_CREATE_PARAM.parent_position,
            ams_AME_CREATE_PARAM.position);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_OMNI.position, ams_AME_NODE_OMNI.translate);
        AppMain.amVectorScale(ams_AME_RUNTIME_WORK_OMNI.velocity, ams_AME_CREATE_PARAM.parent_velocity,
            ams_AME_NODE_OMNI.inheritance_rate);
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) r;
        AMS_AME_NODE_OMNI ams_AME_NODE_OMNI = (AMS_AME_NODE_OMNI) ams_AME_RUNTIME.node;
        AMS_AME_RUNTIME_WORK_OMNI ams_AME_RUNTIME_WORK_OMNI = (AMS_AME_RUNTIME_WORK_OMNI) ams_AME_RUNTIME.work;
        ...
        NNS_VECTOR4D amEffect_vel = AppMain._amEffect_vel;
        ...
        ams_AME_RUNTIME_WORK_OMNI.offset = ams_AME_NODE_OMNI.offset * size_rate;
        ams_AME_RUNTIME_WORK_OMNI.offset_chaos = ams_AME_NODE_OMNI.offset_chaos * size_rate;
        NNS_VECTOR4D amEffect_position = AppMain._amEffect_position;
        NNS_VECTOR4D amEffect_velocity = AppMain._amEffect_velocity;
        NNS_VECTOR4D amEffect_direction = AppMain._amEffect_direction;
        AMS_AME_LIST next = ams_AME_RUNTIME.child_head.next;
        AMS_AME_LIST child_tail = ams_AME_RUNTIME.child_tail;
        while (next != child_tail)
        {
            AMS_AME_RUNTIME ams_AME_RUNTIME2 = (AMS_AME_RUNTIME) next;
            ...
            while (ams_AME_RUNTIME2.amount >= 1f)
            {
                ...
                if (ams_AME_NODE_OMNI.max_count != -1f &&
                    (float) (ams_AME_RUNTIME2.work_num + ams_AME_RUNTIME2.active_num) < ams_AME_NODE_OMNI.max_count)
                {
                    AMS_AME_CREATE_PARAM amEffect_create_param = AppMain._amEffect_create_param;
                    amEffect_create_param.Clear();
                    AppMain.amVectorRandom(amEffect_direction);
                    AppMain.amVectorScale(amEffect_velocity, amEffect_direction,
                        ams_AME_RUNTIME_WORK_OMNI.offset + ams_AME_RUNTIME_WORK_OMNI.offset_chaos * AppMain.nnRandom());
                    amEffect_position.Assign(amEffect_velocity);
                    AppMain.amVectorScale(amEffect_velocity, amEffect_direction,
                        ams_AME_NODE_OMNI.speed + ams_AME_NODE_OMNI.speed_chaos * AppMain.nnRandom());
                    ...
                    int num = AmAme.SUPER_CLASS_ID(ams_AME_RUNTIME2.node);
                    if (num != 256)
                    {
                        if (num == 512)
                        {
    ...
    {
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = (AMS_AME_CREATE_PARAM) p;
        AMS_AME_NODE_DIRECTIONAL ams_AME_NODE_DIRECTIONAL = (AMS_AME_NODE_DIRECTIONAL) ams_AME_CREATE_PARAM.node;
        AMS_AME_RUNTIME_WORK_DIRECTIONAL ams_AME_RUNTIME_WORK_DIRECTIONAL =
            (AMS_AME_RUNTIME_WORK_DIRECTIONAL) ams_AME_CREATE_PARAM.work;
        ams_AME_RUNTIME_WORK_DIRECTIONAL.time = -ams_AME_NODE_DIRECTIONAL.start_time;
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_DIRECTIONAL.position, ams_AME_CREATE_PARAM.parent_position,
            ams_AME_CREATE_PARAM.position);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_DIRECTIONAL.position, ams_AME_NODE_DIRECTIONAL.translate);
        AppMain.amVectorScale(ams_AME_RUNTIME_WORK_DIRECTIONAL.velocity, ams_AME_CREATE_PARAM.parent_velocity,
            ams_AME_NODE_DIRECTIONAL.inheritance_rate);
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) r;
        AMS_AME_NODE_DIRECTIONAL ams_AME_NODE_DIRECTIONAL = (AMS_AME_NODE_DIRECTIONAL) ams_AME_RUNTIME.node;
        AMS_AME_RUNTIME_WORK_DIRECTIONAL ams_AME_RUNTIME_WORK_DIRECTIONAL =
            new AMS_AME_RUNTIME_WORK_DIRECTIONAL(ams_AME_RUNTIME.work);
        ...
        if (ams_AME_NODE_DIRECTIONAL.life != -1f &&
            ams_AME_RUNTIME_WORK_DIRECTIONAL.time >= ams_AME_NODE_DIRECTIONAL.life)
        {
            return 1;
        }
        ...
        NNS_MATRIX amEffect_mtx = AppMain._amEffect_mtx;
        AppMain.nnMakeUnitMatrix(amEffect_mtx);
        AppMain.amMatrixPush(amEffect_mtx);
        NNS_QUATERNION rotate = ams_AME_RUNTIME_WORK_DIRECTIONAL.rotate;
        ...
        while (next != child_tail)
        {
            ...
            while (ams_AME_RUNTIME2.amount >= 1f)
            {
                ams_AME_RUNTIME2.amount -= 1f;
                ams_AME_RUNTIME2.count += 1U;
                if (ams_AME_NODE_DIRECTIONAL.max_count != -1f &&
                    (float) (ams_AME_RUNTIME2.work_num + ams_AME_RUNTIME2.active_num) <
                    ams_AME_NODE_DIRECTIONAL.max_count)
                {
                    AMS_AME_CREATE_PARAM amEffect_create_param = AppMain._amEffect_create_param;
                    amEffect_create_param.Clear();
                    ...
                    AppMain.amVectorScale(amEffect_velocity, amEffect_direction,
                        ams_AME_NODE_DIRECTIONAL.offset + ams_AME_NODE_DIRECTIONAL.offset_chaos * AppMain.nnRandom());
                    amEffect_position.Assign(amEffect_velocity);
                    AppMain.amVectorScale(amEffect_velocity, amEffect_direction,
                        ams_AME_NODE_DIRECTIONAL.speed + ams_AME_NODE_DIRECTIONAL.speed_chaos * AppMain.nnRandom());
                    amEffect_create_param.ecb = ams_AME_RUNTIME.ecb;
                    amEffect_create_param.runtime = ams_AME_RUNTIME2;
                    amEffect_create_param.node = ams_AME_RUNTIME2.node;
    ...
    {
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = (AMS_AME_CREATE_PARAM) p;
        AMS_AME_NODE_SURFACE ams_AME_NODE_SURFACE = (AMS_AME_NODE_SURFACE) ams_AME_CREATE_PARAM.node;
        AMS_AME_RUNTIME_WORK_SURFACE ams_AME_RUNTIME_WORK_SURFACE =
            (AMS_AME_RUNTIME_WORK_SURFACE) ams_AME_CREATE_PARAM.work;
        ams_AME_RUNTIME_WORK_SURFACE.time = -ams_AME_NODE_SURFACE.start_time;
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_SURFACE.position, ams_AME_CREATE_PARAM.parent_position,
            ams_AME_CREATE_PARAM.position);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_SURFACE.position, ams_AME_NODE_SURFACE.translate);
        AppMain.amVectorScale(ams_AME_RUNTIME_WORK_SURFACE.velocity, ams_AME_CREATE_PARAM.parent_velocity,
            ams_AME_NODE_SURFACE.inheritance_rate);
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) r;
        AMS_AME_NODE_SURFACE ams_AME_NODE_SURFACE = (AMS_AME_NODE_SURFACE) ams_AME_RUNTIME.node;
        AMS_AME_RUNTIME_WORK_SURFACE ams_AME_RUNTIME_WORK_SURFACE = (AMS_AME_RUNTIME_WORK_SURFACE) ams_AME_RUNTIME.work;
        ...
        AppMain.nnMakeUnitMatrix(amEffect_mtx);
        AppMain.amMatrixPush(amEffect_mtx);
        NNS_QUATERNION rotate = ams_AME_RUNTIME_WORK_SURFACE.rotate;
        AppMain.amQuatToMatrix(null, ref rotate, null);
        ...
        while (next != child_tail)
        {
            ...
            while (ams_AME_RUNTIME2.amount >= 1f)
            {
                ams_AME_RUNTIME2.amount -= 1f;
                ams_AME_RUNTIME2.count += 1U;
                if (ams_AME_NODE_SURFACE.max_count != -1f &&
                    (float) (ams_AME_RUNTIME2.work_num + ams_AME_RUNTIME2.active_num) < ams_AME_NODE_SURFACE.max_count)
                {
                    AMS_AME_CREATE_PARAM amEffect_create_param = AppMain._amEffect_create_param;
                    ...
                    AppMain.amMatrixCalcPoint(amEffect_direction, amEffect_direction);
                    AppMain.amVectorScale(amEffect_velocity, amEffect_direction,
                        ams_AME_RUNTIME_WORK_SURFACE.offset +
                        ams_AME_RUNTIME_WORK_SURFACE.offset_chaos * AppMain.nnRandom());
                    AppMain.amVectorAdd(amEffect_position, amEffect_velocity);
                    AppMain.amVectorScale(amEffect_velocity, amEffect_direction,
                        ams_AME_NODE_SURFACE.speed + ams_AME_NODE_SURFACE.speed_chaos * AppMain.nnRandom());
                    amEffect_create_param.ecb = ams_AME_RUNTIME.ecb;
                    amEffect_create_param.runtime = ams_AME_RUNTIME2;
                    amEffect_create_param.node = ams_AME_RUNTIME2.node;
                    ...
                    amEffect_create_param.position = amEffect_position;
                    amEffect_create_param.velocity = amEffect_velocity;
                    int num5 = AmAme.NODE_TYPE(ams_AME_RUNTIME2.node) & 65280;
                    if (num5 != 256)
                    {
                        if (num5 == 512)
                        {
    ...
    {
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = (AMS_AME_CREATE_PARAM) p;
        AMS_AME_NODE_CIRCLE ams_AME_NODE_CIRCLE = (AMS_AME_NODE_CIRCLE) ams_AME_CREATE_PARAM.node;
        AMS_AME_RUNTIME_WORK_CIRCLE ams_AME_RUNTIME_WORK_CIRCLE =
            (AMS_AME_RUNTIME_WORK_CIRCLE) ams_AME_CREATE_PARAM.work;
        ams_AME_RUNTIME_WORK_CIRCLE.time = -ams_AME_NODE_CIRCLE.start_time;
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_CIRCLE.position, ams_AME_CREATE_PARAM.parent_position,
            ams_AME_CREATE_PARAM.position);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_CIRCLE.position, ams_AME_NODE_CIRCLE.translate);
        AppMain.amVectorScale(ams_AME_RUNTIME_WORK_CIRCLE.velocity, ams_AME_CREATE_PARAM.parent_velocity,
            ams_AME_NODE_CIRCLE.inheritance_rate);
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) rr;
        AMS_AME_NODE_CIRCLE ams_AME_NODE_CIRCLE = (AMS_AME_NODE_CIRCLE) ams_AME_RUNTIME.node;
        AMS_AME_RUNTIME_WORK_CIRCLE ams_AME_RUNTIME_WORK_CIRCLE = (AMS_AME_RUNTIME_WORK_CIRCLE) ams_AME_RUNTIME.work;
        ...
        ams_AME_RUNTIME_WORK_CIRCLE.offset = ams_AME_NODE_CIRCLE.offset * size_rate;
        ams_AME_RUNTIME_WORK_CIRCLE.offset_chaos = ams_AME_NODE_CIRCLE.offset_chaos * size_rate;
        NNS_MATRIX amUpdateCircle_mtx = AppMain._amUpdateCircle_mtx;
        AppMain.nnMakeUnitMatrix(amUpdateCircle_mtx);
        AppMain.amMatrixPush(amUpdateCircle_mtx);
        NNS_QUATERNION rotate = ams_AME_RUNTIME_WORK_CIRCLE.rotate;
        AppMain.amQuatToMatrix(null, ref rotate, null);
        ams_AME_RUNTIME_WORK_CIRCLE.rotate = rotate;
        NNS_VECTOR4D nns_VECTOR4D2 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        NNS_VECTOR4D nns_VECTOR4D3 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        NNS_VECTOR4D nns_VECTOR4D4 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        AMS_AME_LIST next = ams_AME_RUNTIME.child_head.next;
        AMS_AME_LIST child_tail = ams_AME_RUNTIME.child_tail;
        while (next != child_tail)
        {
            ...
            while (ams_AME_RUNTIME2.amount >= 1f)
            {
                ams_AME_RUNTIME2.amount -= 1f;
                ams_AME_RUNTIME2.count += 1U;
                if (ams_AME_NODE_CIRCLE.max_count != -1f &&
                    (float) (ams_AME_RUNTIME2.work_num + ams_AME_RUNTIME2.active_num) < ams_AME_NODE_CIRCLE.max_count)
                {
                    AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = new AMS_AME_CREATE_PARAM();
                    ...
                    else if ((ams_AME_NODE_CIRCLE.flag & 2U) != 0U)
                    {
                        ...
                        angle = (int) ((long) (65535 / num2) *
                                       (long) ((ulong) ams_AME_RUNTIME2.count % (ulong) ((long) num2)));
                    }
                    ...
                    NNS_VECTOR4D nns_VECTOR4D5 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
                    NNS_QUATERNION nns_QUATERNION = default(NNS_QUATERNION);
                    AppMain.amVectorOuterProduct(nns_VECTOR4D5, nns_VECTOR4D4, nns_VECTOR4D2);
                    AppMain.amVectorUnit(nns_VECTOR4D5);
                    AppMain.amQuatRotAxisToQuat(ref nns_QUATERNION, nns_VECTOR4D5,
                        AppMain.NNM_DEGtoRAD((int) ams_AME_RUNTIME_WORK_CIRCLE.spread));
                    AppMain.amQuatMultiVector(nns_VECTOR4D4, nns_VECTOR4D4, ref nns_QUATERNION, null);
                    AppMain.GlobalPool<NNS_VECTOR4D>.Release(nns_VECTOR4D5);
                    AppMain.amMatrixCalcVector(nns_VECTOR4D2, nns_VECTOR4D2);
                    AppMain.amMatrixCalcVector(nns_VECTOR4D4, nns_VECTOR4D4);
                    AppMain.amVectorScale(nns_VECTOR4D3, nns_VECTOR4D4,
                        ams_AME_RUNTIME_WORK_CIRCLE.offset +
                        ams_AME_RUNTIME_WORK_CIRCLE.offset_chaos * AppMain.nnRandom());
                    AppMain.amVectorAdd(nns_VECTOR4D2, nns_VECTOR4D3);
                    AppMain.amVectorScale(nns_VECTOR4D3, nns_VECTOR4D4,
                        ams_AME_NODE_CIRCLE.speed + ams_AME_NODE_CIRCLE.speed_chaos * AppMain.nnRandom());
        ...
        AppMain.GlobalPool<NNS_VECTOR4D>.Release(nns_VECTOR4D3);
        AppMain.GlobalPool<NNS_VECTOR4D>.Release(nns_VECTOR4D4);
        AppMain.amMatrixPop();
        return 0;
    }
    ...
    {
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = (AMS_AME_CREATE_PARAM) p;
        AMS_AME_NODE_SPRITE ams_AME_NODE_SPRITE = (AMS_AME_NODE_SPRITE) ams_AME_CREATE_PARAM.node;
        AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE ams_AME_RUNTIME_WORK_SIMPLE_SPRITE =
            (AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE) ams_AME_CREATE_PARAM.work;
        ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.time = -ams_AME_NODE_SPRITE.start_time;
        ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.set_color(ams_AME_NODE_SPRITE.color_start.r,
            ams_AME_NODE_SPRITE.color_start.g, ams_AME_NODE_SPRITE.color_start.b,
            (byte) ((int) ams_AME_NODE_SPRITE.color_start.a * ams_AME_CREATE_PARAM.ecb.transparency >> 8));
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.position, ams_AME_CREATE_PARAM.parent_position,
            ams_AME_CREATE_PARAM.position);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.position, ams_AME_NODE_SPRITE.translate);
        AppMain.amVectorScale(ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.velocity, ams_AME_CREATE_PARAM.parent_velocity,
            ams_AME_NODE_SPRITE.inheritance_rate);
        ...
        ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.set_size(num * ams_AME_NODE_SPRITE.scale_x_start,
            num * ams_AME_NODE_SPRITE.scale_y_start, num, 0f);
        if ((ams_AME_NODE_SPRITE.flag & 32768U) != 0U)
        {
            ...
            {
                ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.tex_no =
                    (int) (100f * AppMain.nnRandom()) % ams_AME_NODE_SPRITE.tex_anim.key_num;
            }

            AMS_AME_TEX_ANIM_KEY ams_AME_TEX_ANIM_KEY =
                ams_AME_NODE_SPRITE.tex_anim.key_buf[ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.tex_no];
            Vector4 vector = new Vector4(ams_AME_TEX_ANIM_KEY.l, ams_AME_TEX_ANIM_KEY.t, ams_AME_TEX_ANIM_KEY.r,
                ams_AME_TEX_ANIM_KEY.b);
            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.set_st(vector.X, vector.Y, vector.Z, vector.W);
        }
        else if ((ams_AME_NODE_SPRITE.flag & 8192U) != 0U)
        {
            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.set_st(ams_AME_NODE_SPRITE.cropping_l, ams_AME_NODE_SPRITE.cropping_t,
                ams_AME_NODE_SPRITE.cropping_r, ams_AME_NODE_SPRITE.cropping_b);
        }
        else
        {
        ...
        if ((ams_AME_NODE_SPRITE.flag & 1048576U) != 0U ||
            ((ams_AME_NODE_SPRITE.flag & 131072U) != 0U && AppMain.nnRandom() > 0.5f))
        {
            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.flag |= 8U;
            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.set_st(ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.st.z,
                ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.st.y, ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.st.x,
                ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.st.w);
        }

        if ((ams_AME_NODE_SPRITE.flag & 2097152U) != 0U ||
            ((ams_AME_NODE_SPRITE.flag & 262144U) != 0U && AppMain.nnRandom() > 0.5f))
        {
            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.flag |= 16U;
            ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.set_st(ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.st.x,
                ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.st.w, ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.st.z,
                ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.st.y);
        }
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) r;
        AMS_AME_NODE_SPRITE ams_AME_NODE_SPRITE = (AMS_AME_NODE_SPRITE) ams_AME_RUNTIME.node;
        AMS_AME_LIST next = ams_AME_RUNTIME.active_head.next;
        AMS_AME_LIST active_tail = ams_AME_RUNTIME.active_tail;
        ...
        {
            AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE
                work = (AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE) ((AMS_AME_RUNTIME_WORK) next);
            ...
            if (work.time >= num)
            {
                ...
                AppMain.amEffectDisconnectLink((AMS_AME_LIST) work);
                AMS_AME_RUNTIME ams_AME_RUNTIME2 = ams_AME_RUNTIME;
                ams_AME_RUNTIME2.active_num -= 1;
                AppMain.amEffectFreeRuntimeWork(work);
            }
            else
            {
                ...
                Vector4D_Buf size = work.size;
                work.set_size(size.z * num9, size.z * num10, size.z, size.w);
                AMS_RGBA8888 ams_RGBA;
                AppMain.amEffectLerpColor(out ams_RGBA, ref ams_AME_NODE_SPRITE.color_start,
                    ref ams_AME_NODE_SPRITE.color_end, num7);
                ams_RGBA.a = (byte) ((int) ams_RGBA.a * transparency >> 8);
                work.set_color(ams_RGBA.color);
                if ((ams_AME_NODE_SPRITE.flag & 32768U) != 0U)
                {
                    AMS_AME_TEX_ANIM tex_anim = ams_AME_NODE_SPRITE.tex_anim;
                    if ((work.flag & 2U) == 0U)
                    {
                    ...
                    AMS_AME_TEX_ANIM_KEY ams_AME_TEX_ANIM_KEY = tex_anim.key_buf[work.tex_no];
                    Vector4 vector = new Vector4(ams_AME_TEX_ANIM_KEY.l, ams_AME_TEX_ANIM_KEY.t, ams_AME_TEX_ANIM_KEY.r,
                        ams_AME_TEX_ANIM_KEY.b);
                    if ((work.flag & 8U) != 0U)
                    {
    ...
    public static int _amDrawSimpleSprite(object r)
    {
        ...
        AMS_AME_NODE_SPRITE ams_AME_NODE_SPRITE = (AMS_AME_NODE_SPRITE) ams_AME_RUNTIME.node;
        AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE ams_AME_RUNTIME_WORK_SIMPLE_SPRITE =
            (AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE) ((AMS_AME_RUNTIME_WORK) ams_AME_RUNTIME.active_head.next);
        AMS_AME_LIST next = ams_AME_RUNTIME.active_head.next;
        AMS_AME_LIST active_tail = ams_AME_RUNTIME.active_tail;
        ...
        AppMain.amVectorSet(ref snns_VECTOR, z_bias * AppMain._am_ef_worldViewMtx.M20,
            z_bias * AppMain._am_ef_worldViewMtx.M21, z_bias * AppMain._am_ef_worldViewMtx.M22);
        AppMain.amVectorSet(ref snns_VECTOR2, AppMain._am_ef_worldViewMtx.M10, AppMain._am_ef_worldViewMtx.M11,
            AppMain._am_ef_worldViewMtx.M12);
        AppMain.amVectorSet(ref snns_VECTOR3, AppMain._am_ef_worldViewMtx.M00, AppMain._am_ef_worldViewMtx.M01,
            AppMain._am_ef_worldViewMtx.M02);
        SNNS_VECTOR snns_VECTOR4 = default(SNNS_VECTOR);
        SNNS_VECTOR snns_VECTOR5 = default(SNNS_VECTOR);
        SNNS_VECTOR snns_VECTOR6 = default(SNNS_VECTOR);
        if ((ams_AME_NODE_SPRITE.flag & 4096U) != 0U)
        {
            AppMain.NNS_PRIM3D_PCT_ARRAY nns_PRIM3D_PCT_ARRAY =
                AppMain.amDrawAlloc_NNS_PRIM3D_PCT((int) (6 * ams_AME_RUNTIME.active_num));
            AppMain.NNS_PRIM3D_PCT[] buffer = nns_PRIM3D_PCT_ARRAY.buffer;
            int num = nns_PRIM3D_PCT_ARRAY.offset;
            float sortZ = 0f;
            while (next != active_tail)
            {
                ams_AME_RUNTIME_WORK_SIMPLE_SPRITE = (AMS_AME_RUNTIME_WORK_SIMPLE_SPRITE) ((AMS_AME_RUNTIME_WORK) next);
                ...
                buffer[num + 5].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.color.r,
                    ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.color.g, ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.color.b,
                    ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.color.a);
                buffer[num].Col = (buffer[num + 1].Col = (buffer[num + 2].Col = buffer[num + 5].Col));
                Vector4D_Quat st = ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.st;
        ...
        else
        {
            ...
            while (next != active_tail)
            {
                ...
                array[num2 + 5].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.color.r,
                    ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.color.g, ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.color.b,
                    ams_AME_RUNTIME_WORK_SIMPLE_SPRITE.color.a);
    ...
    public static int _amInitSprite(object p)
    {
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = (AMS_AME_CREATE_PARAM) p;
        AMS_AME_NODE_SPRITE ams_AME_NODE_SPRITE = (AMS_AME_NODE_SPRITE) ams_AME_CREATE_PARAM.node;
        AMS_AME_RUNTIME_WORK_SPRITE ams_AME_RUNTIME_WORK_SPRITE =
            (AMS_AME_RUNTIME_WORK_SPRITE) ams_AME_CREATE_PARAM.work;
        ams_AME_RUNTIME_WORK_SPRITE.time = -ams_AME_NODE_SPRITE.start_time;
        AMS_RGBA8888 color_start = ams_AME_NODE_SPRITE.color_start;
        ...
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_SPRITE.position, ams_AME_CREATE_PARAM.parent_position,
            ams_AME_CREATE_PARAM.position);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_SPRITE.position, ams_AME_NODE_SPRITE.translate);
        AppMain.amVectorScale(ams_AME_RUNTIME_WORK_SPRITE.velocity, ams_AME_CREATE_PARAM.parent_velocity,
            ams_AME_NODE_SPRITE.inheritance_rate);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_SPRITE.velocity, ams_AME_CREATE_PARAM.velocity);
        float num = ams_AME_NODE_SPRITE.size + ams_AME_NODE_SPRITE.size_chaos * AppMain.nnRandom();
        ams_AME_RUNTIME_WORK_SPRITE.set_size(num * ams_AME_NODE_SPRITE.scale_x_start,
            num * ams_AME_NODE_SPRITE.scale_x_start, num, 0f);
        ams_AME_RUNTIME_WORK_SPRITE.twist = ams_AME_NODE_SPRITE.twist_angle +
                                            ams_AME_NODE_SPRITE.twist_angle_chaos * AppMain.nnRandom();
        ...
        if ((ams_AME_NODE_SPRITE.flag & 32768U) != 0U)
        {
            ...
            if ((ams_AME_NODE_SPRITE.flag & 524288U) != 0U)
            {
                ams_AME_RUNTIME_WORK_SPRITE.tex_no =
                    (int) (100f * AppMain.nnRandom()) % ams_AME_NODE_SPRITE.tex_anim.key_num;
            }

            AMS_AME_TEX_ANIM_KEY ams_AME_TEX_ANIM_KEY =
                ams_AME_NODE_SPRITE.tex_anim.key_buf[ams_AME_RUNTIME_WORK_SPRITE.tex_no];
            ams_AME_RUNTIME_WORK_SPRITE.set_st(ams_AME_TEX_ANIM_KEY.l, ams_AME_TEX_ANIM_KEY.t, ams_AME_TEX_ANIM_KEY.r,
                ams_AME_TEX_ANIM_KEY.b);
        }
        else if ((ams_AME_NODE_SPRITE.flag & 8192U) != 0U)
        {
            ams_AME_RUNTIME_WORK_SPRITE.set_st(ams_AME_NODE_SPRITE.cropping_l, ams_AME_NODE_SPRITE.cropping_t,
                ams_AME_NODE_SPRITE.cropping_r, ams_AME_NODE_SPRITE.cropping_b);
        }
        else
        {
            ...
            ((ams_AME_NODE_SPRITE.flag & 131072U) != 0U && AppMain.nnRandom() > 0.5f))
        {
            ...
            ams_AME_RUNTIME_WORK_SPRITE.set_st(ams_AME_RUNTIME_WORK_SPRITE.st.z, ams_AME_RUNTIME_WORK_SPRITE.st.y,
                ams_AME_RUNTIME_WORK_SPRITE.st.x, ams_AME_RUNTIME_WORK_SPRITE.st.w);
        }
            ...
            ((ams_AME_NODE_SPRITE.flag & 262144U) != 0U && AppMain.nnRandom() > 0.5f))
        {
            ...
            ams_AME_RUNTIME_WORK_SPRITE.set_st(ams_AME_RUNTIME_WORK_SPRITE.st.x, ams_AME_RUNTIME_WORK_SPRITE.st.w,
                ams_AME_RUNTIME_WORK_SPRITE.st.z, ams_AME_RUNTIME_WORK_SPRITE.st.y);
        }
    ...
    public static int _amUpdateSprite(object r)
    {
        ...
        while (next != active_tail)
        {
            AMS_AME_RUNTIME_WORK_SPRITE work = (AMS_AME_RUNTIME_WORK_SPRITE) ((AMS_AME_RUNTIME_WORK) next);
            work.time += AppMain._am_unit_frame;
            float num7 = work.time * num2;
            ...
            else
            {
                ...
                if ((ams_AME_NODE_SPRITE.flag & 32768U) != 0U)
                {
                    ...
                    AMS_AME_TEX_ANIM_KEY ams_AME_TEX_ANIM_KEY = tex_anim.key_buf[work.tex_no];
                    work.set_st(ams_AME_TEX_ANIM_KEY.l, ams_AME_TEX_ANIM_KEY.t, ams_AME_TEX_ANIM_KEY.r,
                        ams_AME_TEX_ANIM_KEY.b);
                    if ((work.flag & 8U) != 0U)
                    {
    ...
    public static int _amDrawSprite(object r)
    {
        ...
        if ((ams_AME_NODE_SPRITE.flag & 4096U) != 0U)
        {
            ...
            {
                AMS_AME_RUNTIME_WORK_SPRITE ams_AME_RUNTIME_WORK_SPRITE =
                    (AMS_AME_RUNTIME_WORK_SPRITE) ((AMS_AME_RUNTIME_WORK) next);
                float num2;
                float num3;
                ...
                AppMain.nnSubtractVector(ref buffer[num + 5].Pos, ref buffer[num + 5].Pos, ref snns_VECTOR6);
                buffer[num + 5].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_SPRITE.color.r,
                    ams_AME_RUNTIME_WORK_SPRITE.color.g, ams_AME_RUNTIME_WORK_SPRITE.color.b,
                    ams_AME_RUNTIME_WORK_SPRITE.color.a);
                buffer[num].Col = (buffer[num + 1].Col = (buffer[num + 2].Col = buffer[num + 5].Col));
        ...
        else
        {
            ...
            while (next != active_tail)
            {
                ...
                array[num4 + 5].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_SPRITE.color.r,
                    ams_AME_RUNTIME_WORK_SPRITE.color.g, ams_AME_RUNTIME_WORK_SPRITE.color.b,
                    ams_AME_RUNTIME_WORK_SPRITE.color.a);
    ...
    {
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = (AMS_AME_CREATE_PARAM) p;
        AMS_AME_NODE_LINE ams_AME_NODE_LINE = (AMS_AME_NODE_LINE) ams_AME_CREATE_PARAM.node;
        AMS_AME_RUNTIME_WORK_LINE ams_AME_RUNTIME_WORK_LINE = (AMS_AME_RUNTIME_WORK_LINE) ams_AME_CREATE_PARAM.work;
        ...
        AMS_RGBA8888 ams_RGBA = ams_AME_NODE_LINE.inside_color_start;
        ...
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_LINE.position, ams_AME_CREATE_PARAM.parent_position,
            ams_AME_CREATE_PARAM.position);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_LINE.position, ams_AME_NODE_LINE.translate);
        AppMain.amVectorScale(ams_AME_RUNTIME_WORK_LINE.velocity, ams_AME_CREATE_PARAM.parent_velocity,
            ams_AME_NODE_LINE.inheritance_rate);
        ...
        if ((ams_AME_NODE_LINE.flag & 32768U) != 0U)
        {
            ...
            {
                ams_AME_RUNTIME_WORK_LINE.tex_no =
                    (int) (100f * AppMain.nnRandom()) % ams_AME_NODE_LINE.tex_anim.key_num;
            }

            AMS_AME_TEX_ANIM_KEY ams_AME_TEX_ANIM_KEY =
                ams_AME_NODE_LINE.tex_anim.key_buf[ams_AME_RUNTIME_WORK_LINE.tex_no];
            ams_AME_RUNTIME_WORK_LINE.set_st(ams_AME_TEX_ANIM_KEY.l, ams_AME_TEX_ANIM_KEY.t, ams_AME_TEX_ANIM_KEY.r,
                ams_AME_TEX_ANIM_KEY.b);
        }
        else if ((ams_AME_NODE_LINE.flag & 8192U) != 0U)
        {
            ams_AME_RUNTIME_WORK_LINE.set_st(ams_AME_NODE_LINE.cropping_l, ams_AME_NODE_LINE.cropping_t,
                ams_AME_NODE_LINE.cropping_r, ams_AME_NODE_LINE.cropping_b);
        }
        else
        {
        ...
        if ((ams_AME_NODE_LINE.flag & 1048576U) != 0U ||
            ((ams_AME_NODE_LINE.flag & 131072U) != 0U && AppMain.nnRandom() > 0.5f))
        {
            ams_AME_RUNTIME_WORK_LINE.flag |= 8U;
            ams_AME_RUNTIME_WORK_LINE.set_st(ams_AME_RUNTIME_WORK_LINE.st.z, ams_AME_RUNTIME_WORK_LINE.st.y,
                ams_AME_RUNTIME_WORK_LINE.st.x, ams_AME_RUNTIME_WORK_LINE.st.w);
        }

        if ((ams_AME_NODE_LINE.flag & 2097152U) != 0U ||
            ((ams_AME_NODE_LINE.flag & 262144U) != 0U && AppMain.nnRandom() > 0.5f))
        {
            ams_AME_RUNTIME_WORK_LINE.flag |= 16U;
            ams_AME_RUNTIME_WORK_LINE.set_st(ams_AME_RUNTIME_WORK_LINE.st.x, ams_AME_RUNTIME_WORK_LINE.st.w,
                ams_AME_RUNTIME_WORK_LINE.st.z, ams_AME_RUNTIME_WORK_LINE.st.y);
        }
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) r;
        AMS_AME_NODE_LINE ams_AME_NODE_LINE = (AMS_AME_NODE_LINE) ams_AME_RUNTIME.node;
        AMS_AME_LIST next = ams_AME_RUNTIME.active_head.next;
        AMS_AME_LIST active_tail = ams_AME_RUNTIME.active_tail;
        int transparency = ams_AME_RUNTIME.ecb.transparency;
        float num;
        ...
        while (next != active_tail)
        {
            AMS_AME_RUNTIME_WORK_LINE work = (AMS_AME_RUNTIME_WORK_LINE) ((AMS_AME_RUNTIME_WORK) next);
            work.time += AppMain._am_unit_frame;
            ...
            else
            {
                ...
                AMS_RGBA8888 ams_RGBA;
                AppMain.amEffectLerpColor(out ams_RGBA, ref ams_AME_NODE_LINE.inside_color_start,
                    ref ams_AME_NODE_LINE.inside_color_end, num9);
                ams_RGBA.a = (byte) ((int) ams_RGBA.a * transparency >> 8);
                work.set_inside_color(ams_RGBA.color);
                AppMain.amEffectLerpColor(out ams_RGBA, ref ams_AME_NODE_LINE.outside_color_start,
                    ref ams_AME_NODE_LINE.outside_color_end, num9);
                ams_RGBA.a = (byte) ((int) ams_RGBA.a * transparency >> 8);
                work.set_outside_color(ams_RGBA.color);
                if ((ams_AME_NODE_LINE.flag & 32768U) != 0U)
                {
                    AMS_AME_TEX_ANIM tex_anim = ams_AME_NODE_LINE.tex_anim;
                    if ((work.flag & 2U) == 0U)
                    {
                        work.tex_time += AppMain._am_unit_frame;
                        if (work.tex_time >= tex_anim.key_buf[work.tex_no].time)
    ...
    public static int _amDrawLine(object r)
    {
        ...
        NNS_VECTOR4D amDrawLine_offset = AppMain._amDrawLine_offset;
        NNS_VECTOR4D amDrawLine_eye = AppMain._amDrawLine_eye;
        float z_bias = ams_AME_NODE_LINE.z_bias;
        AppMain.amVectorSet(amDrawLine_offset, z_bias * AppMain._am_ef_worldViewMtx.M20,
            z_bias * AppMain._am_ef_worldViewMtx.M21, z_bias * AppMain._am_ef_worldViewMtx.M22);
        AppMain.amVectorSet(amDrawLine_eye, AppMain._am_ef_worldViewMtx.M20, AppMain._am_ef_worldViewMtx.M21,
            AppMain._am_ef_worldViewMtx.M22);
        NNS_VECTOR4D amDrawLine_pos = AppMain._amDrawLine_pos0;
        NNS_VECTOR4D amDrawLine_pos2 = AppMain._amDrawLine_pos1;
        NNS_VECTOR amDrawLine_vel = AppMain._amDrawLine_vel;
        NNS_VECTOR amDrawLine_cross = AppMain._amDrawLine_cross;
        if ((ams_AME_NODE_LINE.flag & 4096U) != 0U)
        {
            ...
            while (next != active_tail)
            {
                AMS_AME_RUNTIME_WORK_LINE ams_AME_RUNTIME_WORK_LINE =
                    (AMS_AME_RUNTIME_WORK_LINE) ((AMS_AME_RUNTIME_WORK) next);
                ...
                buffer[num + 1].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_LINE.outside_color.r,
                    ams_AME_RUNTIME_WORK_LINE.outside_color.g, ams_AME_RUNTIME_WORK_LINE.outside_color.b,
                    ams_AME_RUNTIME_WORK_LINE.outside_color.a);
                buffer[num].Col = buffer[num + 1].Col;
                buffer[num + 5].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_LINE.inside_color.r,
                    ams_AME_RUNTIME_WORK_LINE.inside_color.g, ams_AME_RUNTIME_WORK_LINE.inside_color.b,
                    ams_AME_RUNTIME_WORK_LINE.inside_color.a);
                buffer[num + 2].Col = buffer[num + 5].Col;
                Vector4D_Quat st = ams_AME_RUNTIME_WORK_LINE.st;
                buffer[num].Tex.u = st.x;
                buffer[num].Tex.v = st.y;
                buffer[num + 1].Tex.u = st.z;
                buffer[num + 1].Tex.v = st.y;
        ...
        else
        {
            ...
            while (next != active_tail)
            {
                ...
                array[num2 + 1].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_LINE.outside_color.r,
                    ams_AME_RUNTIME_WORK_LINE.outside_color.g, ams_AME_RUNTIME_WORK_LINE.outside_color.b,
                    ams_AME_RUNTIME_WORK_LINE.outside_color.a);
                array[num2].Col = array[num2 + 1].Col;
                array[num2 + 5].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_LINE.inside_color.r,
                    ams_AME_RUNTIME_WORK_LINE.inside_color.g, ams_AME_RUNTIME_WORK_LINE.inside_color.b,
                    ams_AME_RUNTIME_WORK_LINE.inside_color.a);
    ...
    {
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = (AMS_AME_CREATE_PARAM) p;
        AMS_AME_NODE_PLANE ams_AME_NODE_PLANE = (AMS_AME_NODE_PLANE) ams_AME_CREATE_PARAM.node;
        AMS_AME_RUNTIME_WORK_PLANE ams_AME_RUNTIME_WORK_PLANE = (AMS_AME_RUNTIME_WORK_PLANE) ams_AME_CREATE_PARAM.work;
        ams_AME_RUNTIME_WORK_PLANE.time = -ams_AME_NODE_PLANE.start_time;
        AMS_RGBA8888 color_start = ams_AME_NODE_PLANE.color_start;
        ams_AME_RUNTIME_WORK_PLANE.set_color(color_start.r, color_start.g, color_start.b,
            (byte) ((int) color_start.a * ams_AME_CREATE_PARAM.ecb.transparency >> 8));
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_PLANE.position, ams_AME_CREATE_PARAM.parent_position,
            ams_AME_CREATE_PARAM.position);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_PLANE.position, ams_AME_NODE_PLANE.translate);
        AppMain.amVectorScale(ams_AME_RUNTIME_WORK_PLANE.velocity, ams_AME_CREATE_PARAM.parent_velocity,
            ams_AME_NODE_PLANE.inheritance_rate);
        ...
        if ((ams_AME_NODE_PLANE.flag & 4U) != 0U)
        {
            ...
            NNS_QUATERNION rotate = ams_AME_RUNTIME_WORK_PLANE.rotate;
        ...
        if ((ams_AME_NODE_PLANE.flag & 8U) != 0U)
        {
            ...
            ams_AME_RUNTIME_WORK_PLANE.set_rotate_axis(nns_VECTOR4D2.x, nns_VECTOR4D2.y, nns_VECTOR4D2.z,
                ams_AME_NODE_PLANE.rotate_axis.w);
            AppMain.GlobalPool<NNS_VECTOR4D>.Release(nns_VECTOR4D2);
        }
        else
        {
        ...
        ams_AME_RUNTIME_WORK_PLANE.set_size(num * ams_AME_NODE_PLANE.scale_x_start,
            num * ams_AME_NODE_PLANE.scale_y_start, num, 0f);
        if ((ams_AME_NODE_PLANE.flag & 32768U) != 0U)
        {
            ...
            {
                ams_AME_RUNTIME_WORK_PLANE.tex_no =
                    (int) (100f * AppMain.nnRandom()) % ams_AME_NODE_PLANE.tex_anim.key_num;
            }

            AMS_AME_TEX_ANIM_KEY ams_AME_TEX_ANIM_KEY =
                ams_AME_NODE_PLANE.tex_anim.key_buf[ams_AME_RUNTIME_WORK_PLANE.tex_no];
            ams_AME_RUNTIME_WORK_PLANE.set_st(ams_AME_TEX_ANIM_KEY.l, ams_AME_TEX_ANIM_KEY.t, ams_AME_TEX_ANIM_KEY.r,
                ams_AME_TEX_ANIM_KEY.b);
        }
        else if ((ams_AME_NODE_PLANE.flag & 8192U) != 0U)
        {
            ams_AME_RUNTIME_WORK_PLANE.set_st(ams_AME_NODE_PLANE.cropping_l, ams_AME_NODE_PLANE.cropping_t,
                ams_AME_NODE_PLANE.cropping_r, ams_AME_NODE_PLANE.cropping_b);
        }
        else
        {
        ...
        if ((ams_AME_NODE_PLANE.flag & 1048576U) != 0U ||
            ((ams_AME_NODE_PLANE.flag & 131072U) != 0U && AppMain.nnRandom() > 0.5f))
        {
            ams_AME_RUNTIME_WORK_PLANE.flag |= 8U;
            ams_AME_RUNTIME_WORK_PLANE.set_st(ams_AME_RUNTIME_WORK_PLANE.st.z, ams_AME_RUNTIME_WORK_PLANE.st.y,
                ams_AME_RUNTIME_WORK_PLANE.st.x, ams_AME_RUNTIME_WORK_PLANE.st.w);
        }

        if ((ams_AME_NODE_PLANE.flag & 2097152U) != 0U ||
            ((ams_AME_NODE_PLANE.flag & 262144U) != 0U && AppMain.nnRandom() > 0.5f))
        {
            ams_AME_RUNTIME_WORK_PLANE.flag |= 16U;
            ams_AME_RUNTIME_WORK_PLANE.set_st(ams_AME_RUNTIME_WORK_PLANE.st.x, ams_AME_RUNTIME_WORK_PLANE.st.w,
                ams_AME_RUNTIME_WORK_PLANE.st.z, ams_AME_RUNTIME_WORK_PLANE.st.y);
        }
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) r;
        AMS_AME_NODE_PLANE ams_AME_NODE_PLANE = (AMS_AME_NODE_PLANE) ams_AME_RUNTIME.node;
        AMS_AME_LIST next = ams_AME_RUNTIME.active_head.next;
        AMS_AME_LIST active_tail = ams_AME_RUNTIME.active_tail;
        int transparency = ams_AME_RUNTIME.ecb.transparency;
        float num;
        ...
        while (next != active_tail)
        {
            AMS_AME_RUNTIME_WORK_PLANE work = (AMS_AME_RUNTIME_WORK_PLANE) ((AMS_AME_RUNTIME_WORK) next);
            work.time += AppMain._am_unit_frame;
            float num7 = work.time * num2;
            AppMain.amVectorScale(amEffect_vel, work.velocity, AppMain._am_unit_time);
            AppMain.amVectorAdd(work.position, amEffect_vel);
            if (work.time >= num)
            {
                if (ams_AME_RUNTIME.spawn_runtime != null)
                {
                    AppMain._amCreateSpawnParticle(ams_AME_RUNTIME, (AMS_AME_RUNTIME_WORK) ((AMS_AME_LIST) work));
                }

                AppMain.amEffectDisconnectLink((AMS_AME_LIST) work);
                AMS_AME_RUNTIME ams_AME_RUNTIME2 = ams_AME_RUNTIME;
                ams_AME_RUNTIME2.active_num -= 1;
                AppMain.amEffectFreeRuntimeWork((AMS_AME_RUNTIME_WORK) ((AMS_AME_LIST) work));
            }
            else
            {
                ...
                work.set_size(work.size.z * num9, work.size.z * num10, work.size.z, work.size.w);
                AMS_RGBA8888 ams_RGBA;
                AppMain.amEffectLerpColor(out ams_RGBA, ref ams_AME_NODE_PLANE.color_start,
                    ref ams_AME_NODE_PLANE.color_end, num7);
                ams_RGBA.a = (byte) ((int) ams_RGBA.a * transparency >> 8);
                work.set_color(ams_RGBA.color);
                if ((ams_AME_NODE_PLANE.flag & 32768U) != 0U)
                {
                    AMS_AME_TEX_ANIM tex_anim = ams_AME_NODE_PLANE.tex_anim;
                    if ((work.flag & 2U) == 0U)
                    {
                        work.tex_time += AppMain._am_unit_frame;
                        if (work.tex_time >= tex_anim.key_buf[work.tex_no].time)
    ...
    public static int _amDrawPlane(object r)
    {
        ...
        AppMain.amVectorSet(out snns_VECTOR4D, z_bias * AppMain._am_ef_worldViewMtx.M20,
            z_bias * AppMain._am_ef_worldViewMtx.M21, z_bias * AppMain._am_ef_worldViewMtx.M22);
        SNNS_VECTOR snns_VECTOR = default(SNNS_VECTOR);
        if ((ams_AME_NODE_PLANE.flag & 4096U) != 0U)
        {
            ...
            while (next != active_tail)
            {
                AMS_AME_RUNTIME_WORK_PLANE ams_AME_RUNTIME_WORK_PLANE =
                    (AMS_AME_RUNTIME_WORK_PLANE) ((AMS_AME_RUNTIME_WORK) next);
                ...
                buffer[num + 5].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_PLANE.color.r,
                    ams_AME_RUNTIME_WORK_PLANE.color.g, ams_AME_RUNTIME_WORK_PLANE.color.b,
                    ams_AME_RUNTIME_WORK_PLANE.color.a);
                buffer[num].Col = (buffer[num + 1].Col = (buffer[num + 2].Col = buffer[num + 5].Col));
        ...
        else
        {
            ...
            while (next != active_tail)
            {
                ...
                AppMain.amVectorAdd(ref snns_VECTOR, ams_AME_RUNTIME_WORK_PLANE.position, ref snns_VECTOR4D);
                NNS_QUATERNION rotate2 = ams_AME_RUNTIME_WORK_PLANE.rotate;
                ...
                array[num2 + 5].Col = AppMain.AMD_RGBA8888(ams_AME_RUNTIME_WORK_PLANE.color.r,
                    ams_AME_RUNTIME_WORK_PLANE.color.g, ams_AME_RUNTIME_WORK_PLANE.color.b,
                    ams_AME_RUNTIME_WORK_PLANE.color.a);
                array[num2].Col = (array[num2 + 1].Col = (array[num2 + 2].Col = array[num2 + 5].Col));
                array[num2 + 3] = array[num2 + 1];
    ...
    {
        AMS_AME_CREATE_PARAM ams_AME_CREATE_PARAM = (AMS_AME_CREATE_PARAM) p;
        AMS_AME_NODE_MODEL ams_AME_NODE_MODEL = (AMS_AME_NODE_MODEL) ams_AME_CREATE_PARAM.node;
        AMS_AME_RUNTIME_WORK_MODEL ams_AME_RUNTIME_WORK_MODEL = (AMS_AME_RUNTIME_WORK_MODEL) ams_AME_CREATE_PARAM.work;
        ...
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_MODEL.position, ams_AME_CREATE_PARAM.parent_position,
            ams_AME_CREATE_PARAM.position);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_MODEL.position, ams_AME_NODE_MODEL.translate);
        AppMain.amVectorScale(ams_AME_RUNTIME_WORK_MODEL.velocity, ams_AME_CREATE_PARAM.parent_velocity,
            ams_AME_NODE_MODEL.inheritance_rate);
        AppMain.amVectorAdd(ams_AME_RUNTIME_WORK_MODEL.velocity, ams_AME_CREATE_PARAM.velocity);
        if ((ams_AME_NODE_MODEL.flag & 4U) != 0U)
        {
            NNS_VECTOR4D nns_VECTOR4D = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
            AppMain.amVectorRandom(nns_VECTOR4D);
            NNS_QUATERNION rotate = ams_AME_RUNTIME_WORK_MODEL.rotate;
        ...
        if ((ams_AME_NODE_MODEL.flag & 8U) != 0U)
        {
            NNS_VECTOR4D nns_VECTOR4D2 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
            AppMain.amVectorRandom(nns_VECTOR4D2);
            ams_AME_RUNTIME_WORK_MODEL.set_rotate_axis(nns_VECTOR4D2.x, nns_VECTOR4D2.y, nns_VECTOR4D2.z,
                ams_AME_NODE_MODEL.rotate_axis.w);
        }
        else
        {
    ...
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) r;
        AMS_AME_NODE_MODEL ams_AME_NODE_MODEL = (AMS_AME_NODE_MODEL) ams_AME_RUNTIME.node;
        AMS_AME_RUNTIME_WORK_MODEL work =
            (AMS_AME_RUNTIME_WORK_MODEL) ((AMS_AME_RUNTIME_WORK) ams_AME_RUNTIME.active_head.next);
        AMS_AME_LIST next = ams_AME_RUNTIME.active_head.next;
        AMS_AME_LIST active_tail = ams_AME_RUNTIME.active_tail;
        int transparency = ams_AME_RUNTIME.ecb.transparency;
        float num;
        ...
        while (next != active_tail)
        {
            work = (AMS_AME_RUNTIME_WORK_MODEL) ((AMS_AME_RUNTIME_WORK) next);
            work.time += AppMain._am_unit_frame;
            ...
            else
            {
                NNS_QUATERNION nns_QUATERNION = default(NNS_QUATERNION);
                NNS_VECTOR4D nns_VECTOR4D4 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
                Vector4D_Buf rotate_axis = work.rotate_axis;
                ...
                AMS_RGBA8888 ams_RGBA;
                AppMain.amEffectLerpColor(out ams_RGBA, ref ams_AME_NODE_MODEL.color_start,
                    ref ams_AME_NODE_MODEL.color_end, num3);
                ams_RGBA.a = (byte) ((int) ams_RGBA.a * transparency >> 8);
                work.set_color(ams_RGBA.color);
    ...
    public static int _amDrawModel(object r)
    {
        AMS_AME_RUNTIME ams_AME_RUNTIME = (AMS_AME_RUNTIME) r;
        AMS_AME_NODE_MODEL ams_AME_NODE_MODEL = (AMS_AME_NODE_MODEL) ams_AME_RUNTIME.node;
        AMS_AME_ECB ecb = ams_AME_RUNTIME.ecb;
        ...
        while (next != active_tail)
        {
            AMS_AME_RUNTIME_WORK_MODEL ams_AME_RUNTIME_WORK_MODEL =
                (AMS_AME_RUNTIME_WORK_MODEL) ((AMS_AME_RUNTIME_WORK) next);
            AppMain.amMatrixPush();
            SNNS_VECTOR4D snns_VECTOR4D2;
            ...
            NNS_MATRIX nns_MATRIX2 = AppMain.amMatrixGetCurrent();
            nns_MATRIX.Assign(nns_MATRIX2);
            AppMain.nnScaleMatrix(nns_MATRIX, nns_MATRIX, ams_AME_RUNTIME_WORK_MODEL.scale.x,
                ams_AME_RUNTIME_WORK_MODEL.scale.y, ams_AME_RUNTIME_WORK_MODEL.scale.z);
            ...
            AppMain.amDrawObjectSetMaterial(ams_AME_RUNTIME.ecb.drawObjState, ecb.pObj, ams_AME_RUNTIME.texlist,
                nns_VECTOR, ref nns_RGBA, num2, num3, blend, num);
    ...
    public static void _amApplyGravity(AMS_AME_ECB ecb, AMS_AME_NODE node, AMS_AME_RUNTIME_WORK work)
    {
        AMS_AME_NODE_GRAVITY ams_AME_NODE_GRAVITY = (AMS_AME_NODE_GRAVITY) node;
        ...
        AppMain.amVectorScale(nns_VECTOR4D, nns_VECTOR4D2,
            ams_AME_NODE_GRAVITY.magnitude * am_unit_time * am_unit_time * 0.5f);
        AppMain.amVectorAdd(work.position, nns_VECTOR4D);
    ...
    public static void _amApplyUniform(AMS_AME_ECB ecb, AMS_AME_NODE node, AMS_AME_RUNTIME_WORK work)
    {
        AMS_AME_NODE_UNIFORM ams_AME_NODE_UNIFORM = (AMS_AME_NODE_UNIFORM) node;
        float am_unit_time = AppMain._am_unit_time;
        NNS_VECTOR4D nns_VECTOR4D = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        AppMain.amVectorScale(nns_VECTOR4D, ams_AME_NODE_UNIFORM.direction,
            ams_AME_NODE_UNIFORM.magnitude * am_unit_time);
    ...
    public static void _amApplyRadial(AMS_AME_ECB ecb, AMS_AME_NODE node, AMS_AME_RUNTIME_WORK work)
    {
        AMS_AME_NODE_RADIAL ams_AME_NODE_RADIAL = (AMS_AME_NODE_RADIAL) node;
        float am_unit_time = AppMain._am_unit_time;
        NNS_VECTOR4D nns_VECTOR4D = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        NNS_VECTOR4D nns_VECTOR4D2 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        ...
        float num = 1f / (float) Math.Pow((double) AppMain.amVectorScalor(nns_VECTOR4D),
            (double) ams_AME_NODE_RADIAL.attenuation);
    ...
    public static void _amApplyVortex(AMS_AME_ECB ecb, AMS_AME_NODE node, AMS_AME_RUNTIME_WORK work)
    {
        AMS_AME_NODE_VORTEX ams_AME_NODE_VORTEX = (AMS_AME_NODE_VORTEX) node;
        NNS_VECTOR4D nns_VECTOR4D = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        NNS_VECTOR4D nns_VECTOR4D2 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        NNS_VECTOR4D nns_VECTOR4D3 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        NNS_VECTOR4D nns_VECTOR4D4 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
    ...
    public static void _amApplyDrag(AMS_AME_ECB ecb, AMS_AME_NODE node, AMS_AME_RUNTIME_WORK work)
    {
        AMS_AME_NODE_DRAG ams_AME_NODE_DRAG = (AMS_AME_NODE_DRAG) node;
        float am_unit_time = AppMain._am_unit_time;
        NNS_VECTOR4D nns_VECTOR4D = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        NNS_VECTOR4D nns_VECTOR4D2 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        NNS_VECTOR4D nns_VECTOR4D3 = AppMain.GlobalPool<NNS_VECTOR4D>.Alloc();
        ...
        AppMain.amVectorScale(nns_VECTOR4D, nns_VECTOR4D2,
            ams_AME_NODE_DRAG.magnitude * am_unit_time * am_unit_time * 0.5f);
        AppMain.amVectorAdd(work.position, nns_VECTOR4D);
    ...
    public static void _amApplyNoise(AMS_AME_ECB ecb, AMS_AME_NODE node, AMS_AME_RUNTIME_WORK work)
    {
        AMS_AME_NODE_NOISE ams_AME_NODE_NOISE = (AMS_AME_NODE_NOISE) node;
