...
public partial class AppMain
{
    ...
    private static void GmPlayerSetTruckRide(AppMain.GMS_PLAYER_WORK ply_work, AppMain.OBS_OBJECT_WORK truck_obj, short field_left, short field_top, short field_right, short field_bottom)
    {
        ...
        AppMain.ObjRectWorkSet(ply_work.rect_work[2], -8, (short)((short)-32 + field_bottom), 8, (short)field_bottom);
        AppMain.ObjRectWorkSet(ply_work.rect_work[0], -8, (short)((short)-48 + field_bottom), 8, (short)((short)-16 + field_bottom));
    ...
    private static ushort gmPlayerRemapKey(AppMain.GMS_PLAYER_WORK ply_work, ushort key, int key_rot_z)
    {
        ...
        else if ((AppMain.g_gs_main_sys_info.game_flag & 1U) != 0U)
        {
            ...
            ushort value = cpadVirtualPad.IsValid() ? cpadVirtualPad.GetValue() : (ushort)AoPadMDirect();
            if ((8 & value) != 0)
            {
            ...
            if (AppMain.gmPlayerIsInputDPadJumpKey(ply_work, -1) || AoPadADirect(ControllerConsts.JUMP_BUTTON))
            {
                num |= ply_work.key_map[4];
            }
            if (AppMain.gmPlayerIsInputDPadSSonicKey(ply_work, -1) || AoPadADirect(ControllerConsts.SUPER_SONIC))
            {
                num |= ply_work.key_map[6];
            }
        }
