...
public partial class AppMain
{
        ...
        int pos_y, byte type)
    {
        ...
        AppMain.ObjRectSet(obs_RECT_WORK.rect, (short) eve_rec.left, (short) eve_rec.top,
            (short) (eve_rec.width + (byte) eve_rec.left), (short) (eve_rec.height + (byte) eve_rec.top));
        obs_OBJECT_WORK.move_flag |= 8480U;
        obs_OBJECT_WORK.ppFunc = AppMain.gmGmkForceSpinSetMain;
        ...
        int pos_y, byte type)
    {
        ...
        AppMain.ObjRectSet(obs_RECT_WORK.rect, (short) eve_rec.left, (short) eve_rec.top,
            (short) (eve_rec.width + (byte) eve_rec.left), (short) (eve_rec.height + (byte) eve_rec.top));
        obs_OBJECT_WORK.move_flag |= 8480U;
        obs_OBJECT_WORK.ppFunc = AppMain.gmGmkForceSpinResetMain;
        return obs_OBJECT_WORK;
    }
    ...
    private static void gmGmkForceSpinResetMain(AppMain.OBS_OBJECT_WORK obj_work)
    {
        ...
        if (AppMain.gmGmkForceSpinRectChk(obj_work, gms_PLAYER_WORK) &&
            (gms_PLAYER_WORK.seq_state == 51 || gms_PLAYER_WORK.seq_state == 52 || gms_PLAYER_WORK.seq_state == 53))
        {
            ...
            {
                AppMain.GmPlySeqGmkInitSpinFall(gms_PLAYER_WORK, gms_PLAYER_WORK.obj_work.spd.x,
                    gms_PLAYER_WORK.obj_work.spd.y);
            }
        }
    }

    // Token: 0x06001568 RID: 5480 RVA: 0x000B9ECC File Offset: 0x000B80CC
    private static bool gmGmkForceSpinRectChk(OBS_OBJECT_WORK obj_work, GMS_PLAYER_WORK ply_work)
    {
        OBS_RECT_WORK obsRectWork = ((GMS_ENEMY_COM_WORK) obj_work).rect_work[2];
        return ply_work.obj_work.pos.x >= obj_work.pos.x + ((int) obsRectWork.rect.left << 12)
                && ply_work.obj_work.pos.x <= obj_work.pos.x + ((int) obsRectWork.rect.right << 12)
                && (ply_work.obj_work.pos.y >= obj_work.pos.y + ((int) obsRectWork.rect.top << 12)
                && ply_work.obj_work.pos.y <= obj_work.pos.y + ((int) obsRectWork.rect.bottom << 12));
    }
}