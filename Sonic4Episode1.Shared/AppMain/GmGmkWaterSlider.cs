...
public partial class AppMain
{
    ...
    private static OBS_OBJECT_WORK GmGmkWaterSliderInit( AppMain.GMS_EVE_RECORD_EVENT eve_rec, int pos_x, int pos_y, byte type )
    {
    ...
    private static AppMain.GMS_ENEMY_3D_WORK gmGmkWaterSliderLoadObj( AppMain.GMS_EVE_RECORD_EVENT eve_rec, int pos_x, int pos_y, uint type )
    {
        ...
        if ( num3 != -1 )
        {
            ...
            AppMain.ObjAction3dNNMotionLoad( gms_GMK_WATER_SLIDER_WORK.obj_3d_parts, 0, false, null, null, num3, ( AMS_AMB_HEADER )pData2 );
        }
        int num4 = AppMain.g_gm_gmk_water_slider_material_id_sub[(int)((UIntPtr)type)];
        if ( num4 != -1 )
        {
            AppMain.ObjAction3dNNMaterialMotionLoad( gms_GMK_WATER_SLIDER_WORK.obj_3d_parts, 0, null, null, num4, ( AMS_AMB_HEADER )pData );
        }
