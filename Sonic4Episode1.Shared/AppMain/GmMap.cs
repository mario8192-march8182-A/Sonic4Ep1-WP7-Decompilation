...
public partial class AppMain
{
    ...
    private static AppMain.DF_HEADER readDFFile(AmbChunk data)
    {
    ...
    private static AppMain.DI_HEADER readDIFile(AmbChunk data)
    {
    ...
    private static AppMain.AT_HEADER readATFile(AmbChunk data)
    {
    ...
    private static AppMain.MP_HEADER readMPFile(AmbChunk data)
    {
        if (data.length == 0)
        {
            return null;
    ...
    private static AppMain.MD_HEADER readMDFile(AmbChunk data)
    {
        if (data.length == 0)
        {
            return null;
    ...
    private static void GmMapInit()
    {
        ...
        AppMain.gm_map_tcb = AppMain.MTM_TASK_MAKE_TCB(AppMain.gmMapMain, AppMain.gmMapDest, 0U, 0, 12288U, 5,
            () => new AppMain.GMS_MAP_SYS_WORK(), "GM_MAP_MAIN");
    ...
    private static void GmMapDrawMap(AppMain.OBS_ACTION3D_NN_WORK obj3d_tbl, AppMain.MP_HEADER mp_header,
        AppMain.MD_HEADER md_header, float pos_x, float pos_y, float trans_x, float trans_y, float trans_z, bool loop_h)
    {
        float num = 0f;
        float num2 = 0f;
    ...
    private static void gmMapDrawMapRange(AppMain.OBS_ACTION3D_NN_WORK obj3d_tbl, AppMain.MP_HEADER mp_header,
        AppMain.MD_HEADER md_header, float trans_x, float trans_y, float trans_z, int block_left, int block_right,
    ...
    private static void GmMapSetAddMapXLoop()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            if ((gms_MAP_SYS_WORK.flag & 1U << i) != 0U &&
    ...
    private static void GmMapDisenableAddMapUserScrlX()
    {
        ...
        OBS_CAMERA obs_CAMERA = ObjCamera.Get(AppMain.g_obj.glb_camera_id);
        float num = gms_MAP_SYS_WORK.main_cam_user_disp[0] + gms_MAP_SYS_WORK.main_cam_user_ofst[0] -
                    obs_CAMERA.disp_pos.x;
        for (int i = 0; i < 5; i++)
        {
            if (i != 1)
            {
    ...
    public static void GmMapGetAddMapCameraPos(NNS_VECTOR main_disp_pos, NNS_VECTOR main_target_pos,
        NNS_VECTOR dest_disp_pos, NNS_VECTOR dest_target_pos, int camera_id)
    {
        if (AppMain.gm_map_tcb == null)
        {
            AppMain.mppAssertNotImpl();
        ...
        if ((gms_MAP_SYS_WORK.flag & 536870912U) != 0U)
        {
            AppMain.main_camera_pos[0].x =
                gms_MAP_SYS_WORK.main_cam_user_disp[0] + gms_MAP_SYS_WORK.main_cam_user_ofst[0];
            AppMain.main_camera_pos[1].x =
                gms_MAP_SYS_WORK.main_cam_user_target[0] + gms_MAP_SYS_WORK.main_cam_user_ofst[0];
        }
        ...
        {
            dest_disp_pos.y =
                -(num2 + (-AppMain.main_camera_pos[num7].y - num2) * gms_MAP_OTHER_MAP_STATE.scrl_scale[1]);
        }
        ...
        if ((gms_MAP_SYS_WORK.flag & 2147483648U) == 0U)
        {
            ...
            {
                dest_target_pos.x =
                    num + (AppMain.main_camera_pos[num7].x - num) * gms_MAP_OTHER_MAP_STATE.scrl_scale[0];
            }
        }
        else
        {
    ...
    private static void gmMapMain(MTS_TASK_TCB tcb)
    {
        ...
        {
            obs_CAMERA = ObjCamera.Get(AppMain.g_obj.glb_camera_id);
        ...
        for (int i = num3 - 1; i >= num2; i--)
        {
            if ((gms_MAP_SYS_WORK.flag & 1U << i) != 0U && (i < 2 || AppMain.GmMapIsDrawEnableMMapBack()))
            {
                ...
                obs_CAMERA = ObjCamera.Get(AppMain.gm_map_addmap_camera_tbl[i]);
        ...
        if (AppMain.gm_map_tex_draw_count > 0)
        {
            ...
            for (int j = 0; j < AppMain.gm_map_texture.texlist.nTex; j++)
            {
                ...
                buffer[offset].Pos.z = (buffer[offset + 1].Pos.z =
                    (buffer[offset + 2].Pos.z = (buffer[offset + 3].Pos.z = -1f)));
    ...
    private static void gmMapSetDrawMapTvx(AppMain.TVX_FILE[] tvxamb, AppMain.MP_HEADER mp_header,
        AppMain.MD_HEADER md_header, float pos_x, float pos_y, float trans_x, float trans_y, float trans_z, bool loop_h,
        AppMain.MP_BLOCK mp_block, AppMain.MD_BLOCK md_block)
    {
        float num = 0f;
        float num2 = 0f;
        int num3 = (int) pos_x >> 6;
        ...
        if (loop_h)
        {
            AppMain.gmMapSetDrawMapRangeTvx(tvxamb, mp_header, md_header, trans_x + num, trans_y, trans_z, num7, num8,
                num9, num10, mp_block, md_block);
            return;
        }

        AppMain.gmMapSetDrawMapRangeTvx(tvxamb, mp_header, md_header, trans_x, trans_y, trans_z, num7, num8, num9,
            num10, mp_block, md_block);
    }

    // Token: 0x06001896 RID: 6294 RVA: 0x000DFA40 File Offset: 0x000DDC40
    private static void gmMapSetDrawMapRangeTvx(AppMain.TVX_FILE[] tvxamb, AppMain.MP_HEADER mp_header,
        AppMain.MD_HEADER md_header, float trans_x, float trans_y, float trans_z, int block_left, int block_right,
        int block_top, int block_bottom, AppMain.MP_BLOCK _mp_block, AppMain.MD_BLOCK _md_block)
    {
        for (int i = 0; i < 24; i++)
        {
            for (int j = 0; j < 24; j++)
    ...
    private static void gmMapExecuteDrawMapTvxCore(NNS_MATRIX mtx, AppMain.GMS_MAP_PRIM_DRAW_WORK work,
        AppMain.AMS_PARAM_DRAW_PRIMITIVE dat, AppMain.NNS_PRIM3D_PCT_ARRAY v_tbl_array, uint color)
    {
        int num = v_tbl_array.offset;
        AppMain.NNS_PRIM3D_PCT[] buffer = v_tbl_array.buffer;
        dat.vtxPCT3D = v_tbl_array;
        dat.texId = work.tex_id;
        NNS_TEXCOORD nns_TEXCOORD;
    ...
    private static void gmMapBuildDrawMapTvxTexScroll()
    {
        ...
        DoubleType<uint[], AppMain.GMS_MAP_PRIM_DRAW_TVX_MGR_INDEX[]> mgr_index_tbl_addr =
            gms_MAP_PRIM_DRAW_TVX_UV_WORK.mgr_index_tbl_addr;
    ...
    private static void gmMapCreateUsePrimMatrix()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                NNS_MATRIX nns_MATRIX2 = AppMain.gm_map_use_prim_mtx[i * 4 + j];
