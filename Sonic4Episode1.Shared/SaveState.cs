...
using Sonic4Episode1;

// Token: 0x020003E5 RID: 997
public class SaveState
{
    public static string saveStateLocation;
    ...
    public static void _saveFile(object o)
	{
		...
		if (File.Exists(saveStateLocation))
		{
			File.Delete(saveStateLocation);
		}
		using (Stream isolatedStorageFileStream = File.Create(saveStateLocation))
		{
	...
	public static bool isSaveAvailable()
	{
		...
		try
		{
			result = File.Exists(saveStateLocation);
		}
		catch (Exception)
		{
			result = false;
	...
	public static bool loadState()
	{
		...
		try
		{
			if (File.Exists(saveStateLocation))
			{
				using (Stream isolatedStorageFileStream = new FileStream(saveStateLocation, FileMode.Open, FileAccess.Read))
				{
