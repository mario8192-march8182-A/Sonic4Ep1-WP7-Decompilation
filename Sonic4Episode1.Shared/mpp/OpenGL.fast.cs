//
//	Alternative, faster OpenGL implementation for use under MonoGame's DesktopGL profile
//
...
namespace mpp
{
    ...
    public sealed class OpenGL
    {
        ...
        {
            // TODO: do i want a non-gles11 implementation?
            throw new NotImplementedException();
        }
        ...
        public static void glBufferVertexData(GLVertexData data)
        {
            ...
            var vertexBuffer = new VertexBuffer(OpenGL.m_graphicsDevice, OpenGL.Vertex.VertexDeclaration, array.Length, BufferUsage.WriteOnly);
            vertexBuffer.SetData(array);
            vertexBufferDesc.Buffer = vertexBuffer;

            var old = (VertexBufferDesc)OpenGL.m_buffers[OpenGL.m_boundArrayBuffer].buffer;
            if (old != null)
            {
                old.Buffer.Dispose();
            }
        ...
        public static void glBufferIndexData(GLIndexBuffer data)
        {
            ...
            var indexBuffer = new IndexBuffer(OpenGL.m_graphicsDevice, IndexElementSize.SixteenBits, data.Size, BufferUsage.None);
            indexBuffer.SetData(array);
            var indexBuffer2 = (IndexBuffer)OpenGL.m_buffers[OpenGL.m_boundElementArrayBuffer].buffer;
        ...
        public static void glClear(uint mask)
        {
            ...
            var clearOptions = (ClearOptions)0;
            if ((mask & 16384U) != 0U)
            {
        ...
        {
            var colorWriteChannels = ColorWriteChannels.None;
            if (red != 0)
            {
        ...
        public static void glColorPointer(int size, uint type, int stride, GLVertexData pointer)
        {
        ...
        public static void glDeleteBuffers(int n, uint[] buffers)
        {
            for (int i = 0; i < n; i++)
            {
                ...
                if (bufferItem.buffer != null)
                {
                    ...
                    {
                        var vertexBufferDesc = (VertexBufferDesc)bufferItem.buffer;
        ...
        public static void glDrawArrays(uint mode, int first, int count)
        {
            ...
            {
                OpenGL.pendingVertices = new VertexPosTexColNorm[OpenGL.pendingVertices.Length * 2];
            }
        ...
        public static void glDrawElements(uint mode, int count, uint type, ushort[] indices)
        {
            ...
            var bufferItem = OpenGL.m_buffers[OpenGL.m_boundArrayBuffer];
            var vertexBufferDesc = (VertexBufferDesc)bufferItem.buffer;
            if (Array.IndexOf(vertexBufferDesc.Components, OpenGL.GLVertexElementType.Normal) < 0)
            {
                OpenGL.m_ambientColor = new Vector3(1f, 1f, 1f);
            }
            var buffer = vertexBufferDesc.Buffer;
            var ibi = OpenGL.m_buffers[OpenGL.m_boundElementArrayBuffer];
            bool flag = true;
            if ((OpenGL.m_textureUnits[0].TexCoordArrayEnabled && OpenGL.m_textureUnits[0].Enabled) || vertexBufferDesc.vertices == null)
            {
                var matrix = OpenGL.m_textureUnits[0].MatrixStack.Peek();
                var matrix2 = OpenGL.m_textureUnits[1].MatrixStack.Peek();
                ...
                if (flag2 || vertexBufferDesc.vertices == null)
                {
                    ...
                    {
                        vertexBufferDesc.vertices = new Vertex[buffer.VertexCount];
                        flag = true;
                    }
                    Vertex[] array = (Vertex[])bufferItem.rawData;
                    ...
                    if (flag)
                    {
                        buffer.SetData(vertexBufferDesc.vertices);
                    }
                    else
                    {
        ...
        private static void setupEffect(BasicEffect effect, ref DrawArraysCallContext callContext)
        {
        ...
        private static void setupEffect(AlphaTestEffect effect, ref DrawArraysCallContext callContext)
        {
            effect.VertexColorEnabled = callContext.vertexColorEnabled;
            effect.DiffuseColor = callContext.diffuseColor;
            effect.Alpha = callContext.diffuseColorAlpha;
        ...
        private static void setupEffect<TEffect>(TEffect effect, ref DrawArraysCallContext callContext) where TEffect : Effect, IEffectMatrices, IEffectLights, IEffectFog
        {
            effect.Projection = callContext.projection;
            effect.View = callContext.view;
            effect.FogEnabled = callContext.fogEnabled;
        ...
        private static void drawVertexBuffer(PrimitiveType primitiveType, int primitiveCount, BufferItem vbi, BufferItem ibi, bool useVB)
        {
            var vertexBufferDesc = (VertexBufferDesc)vbi.buffer;
            ...
            var blendState = OpenGL.m_blendEnabled ? OpenGL.m_blendState : BlendState.Opaque;
            if (OpenGL.m_graphicsDevice.BlendState != blendState)
            {
                OpenGL.m_graphicsDevice.BlendState = blendState;
            }
            var depthStencilState = OpenGL.m_depthTestEnabled ? OpenGL.m_depthStencilState : DepthStencilState.None;
            ...
            var rasterizerState = OpenGL.m_cullFaceEnabled ? OpenGL.m_rasterizerState : OpenGL.m_rasterizerStateCN;
            ...
            if (OpenGL.m_matrixPalleteOESEnabled)
            {
                ...
                {
                    Vertex[] array = (Vertex[])vbi.rawData;
                    if (vertexBufferDesc.vertices == null)
                    {
                        vertexBufferDesc.vertices = new Vertex[array.Length];
                    }
                    for (int i = 0; i < array.Length; i++)
                    {
                        var vector = array[i].BlendIndices.ToVector4();
            ...
            if (useVB)
            {
                ...
                if (ibi != null)
                {
                    var indices = (IndexBuffer)ibi.buffer;
            ...
            Vertex[] vertices = vertexBufferDesc.vertices;
            foreach (var effectPass2 in effect.CurrentTechnique.Passes)
            {
                effectPass2.Apply();
                if (ibi != null)
                {
                    short[] indexData = (short[])ibi.rawData;
                    OpenGL.m_graphicsDevice.DrawUserIndexedPrimitives(primitiveType, vertices, 0, vertices.Length, indexData, 0, primitiveCount);
                }
                else
                {
                    OpenGL.m_graphicsDevice.DrawUserPrimitives(primitiveType, vertices, 0, primitiveCount);
                }
            }
        }

        // Token: 0x0600227D RID: 8829 RVA: 0x00144CC4 File Offset: 0x00142EC4
        private static void _glDrawArrays_PrepareVertices(PrimitiveType primitiveType, VertexPosTexColNorm[] dstArray, int dstOffset, int count)
        {
            var matrix = OpenGL.m_textureUnits[0].MatrixStack.Peek();
            OpenGL.m_textureUnits[1].MatrixStack.Peek();
            bool flag = matrix.Equals(Matrix.Identity);
            var matrix2 = OpenGL.m_invScreen * Matrix.Invert(OpenGL.m_projectionMatrixStack.Peek());
            var vertexArray = OpenGL.m_vertexArray;
            var colorArray = OpenGL.m_colorArray;
            var normalArray = OpenGL.m_normalArray;
            OpenGL.texCoordData[0] = OpenGL.m_textureUnits[0].TexCoordArray;
            OpenGL.texCoordData[1] = OpenGL.m_textureUnits[1].TexCoordArray;
            var vector = Vector4.Transform(OpenGL.posShift, matrix2 * Matrix.Invert(OpenGL.m_modelViewMatrixStack.Peek()));
            VertexPosTexColNorm[] array;
            int num;
            ...
            var matrix3 = OpenGL.m_modelViewMatrixStack.Peek();
            for (int i = 0; i < count; i++)
            {
                int num2 = num + i;
                VertexPosTexColNorm[] array2 = array;
                int num3 = num2;
        ...
        private static void initVertices(int count, out bool twoPasses)
        {
            ...
            {
                OpenGL.m_vertices[0] = new VertexPosTexColNorm[count * 2];
            }
            var vector = Vector4.Transform(OpenGL.posShift, matrix2 * Matrix.Invert(OpenGL.m_modelViewMatrixStack.Peek()));
            ...
            for (int i = 0; i < count; i++)
            {
                VertexPosTexColNorm[] array = OpenGL.m_vertices[0];
                int num2 = i;
        ...
        private static void drawPrimitives(PrimitiveType primitiveType, int primitiveCount, bool useIndices, bool twoPasses)
        {
            ...
            foreach (var effectPass in effect.CurrentTechnique.Passes)
            {
                ...
                if (useIndices)
                {
                    OpenGL.m_graphicsDevice.DrawUserIndexedPrimitives(primitiveType, OpenGL.m_vertices[0], 0, OpenGL.verticesToDraw_, OpenGL.m_indices, 0, primitiveCount);
                }
                else
                {
                    OpenGL.m_graphicsDevice.DrawUserPrimitives(primitiveType, OpenGL.m_vertices[0], 0, primitiveCount);
                }
            }
        }

        // Token: 0x06002280 RID: 8832 RVA: 0x001453DC File Offset: 0x001435DC
        private static void drawPrimitives(VertexPosTexColNorm[] vertices, PrimitiveType primitiveType, int primitiveCount, ref DrawArraysCallContext callContext)
        {
        ...
        {
            texture = (uint)GLES11.GenTexture();
        }
        ...
        {
            GLES11.LightModel((ES11.LightModelParameter)pname, param);
        }
        ...
        {
            OpenGL.glMaterialfv(face, pname, new glArray4f(param[0], param[1], param[2], param[3]));
        }
        ...
        public static void glMatrixIndexPointerOES(int size, uint type, int stride, GLVertexData data)
        {
        ...
        {
            var matrix = Matrix.CreateOrthographicOffCenter(left, right, bottom, top, nearVal, farVal);
            if (OpenGL.m_matrixPaletteOESActive)
            {
        ...
        {
            var matrix = Matrix.CreateScale(x, y, z);
            if (OpenGL.m_matrixPaletteOESActive)
            {
                OpenGL.m_matrixPalleteOES[(int)((UIntPtr)OpenGL.m_currentPaletteMatrixOES)] = matrix * OpenGL.m_matrixPalleteOES[(int)((UIntPtr)OpenGL.m_currentPaletteMatrixOES)];
                return;
        ...
        {
            GLES11.ShadeModel((ES11.ShadingModel)mode);
        }

        // Token: 0x060022A6 RID: 8870 RVA: 0x00145E14 File Offset: 0x00144014
        public static void glTexCoordPointer(int size, uint type, int stride, GLVertexData pointer)
        {
            var textureUnit = OpenGL.m_textureUnits[(int)((UIntPtr)OpenGL.m_clientActiveTextureUnit)];
        ...
        {
            using (var stream = TitleContainer.OpenStream(Path.Combine("Content", textureName + ".png")))
            {
        ...
        public static void glTexParameteri(uint target, uint pname, int param)
        {
            ...
            var textureAddressMode = OpenGL.m_textureUnits[(int)((UIntPtr)OpenGL.m_activeTextureUnit)].SamplerState.AddressU;
            var textureAddressMode2 = OpenGL.m_textureUnits[(int)((UIntPtr)OpenGL.m_activeTextureUnit)].SamplerState.AddressV;
            switch (pname)
            {
                case 10242U:
                    switch (param)
        ...
        {
            var matrix = Matrix.CreateTranslation(x, y, z);
            if (OpenGL.m_matrixPaletteOESActive)
            {
                OpenGL.m_matrixPalleteOES[(int)((UIntPtr)OpenGL.m_currentPaletteMatrixOES)] = matrix * OpenGL.m_matrixPalleteOES[(int)((UIntPtr)OpenGL.m_currentPaletteMatrixOES)];
                return;
        ...
        public static void glVertexPointer(int size, uint type, int stride, GLVertexData pointer)
        {
        ...
        public static void glWeightPointerOES(int size, uint type, int stride, GLVertexData pointer)
        {
        ...
        public static void init(Game game, GraphicsDevice graphicsDevice)
        {
            ...
            OpenGL.m_fakeTexture.SetData(new Color[]
            {
        ...
        private static LightUnit[] m_lightUnit = new LightUnit[]
        {
            new LightUnit(),
            new LightUnit(),
            new LightUnit()
        ...
        private static TextureUnit[] m_textureUnits = new TextureUnit[]
        {
            new TextureUnit(),
            new TextureUnit()
        };
        ...
        private static VertexPosTexColNorm[][] m_vertices = new VertexPosTexColNorm[][]
        {
