...
public class AmBind
{
    ...
    public static object Get(AMS_AMB_HEADER header, int index)
    {
        string dir;
        AmbChunk ambChunk = Get(header, index, out dir);
        if (header.files[index].IndexOf(".amb", StringComparison.OrdinalIgnoreCase) != -1)
        {
            AMS_AMB_HEADER ams_AMB_HEADER = AmFs.readAMBFile(ambChunk);
    ...
    public static AmbChunk Get(AMS_AMB_HEADER header, int index, out string sPath)
    {
        sPath = null;
        AmbChunk result = null;
        if (index < header.file_num)
        {
            result = new AmbChunk(header.data, header.offsets[index], header.lengths[index], header);
        }
        else
        {
    ...
    public static AmbChunk Get(AMS_FS header, int index)
    {
        string text;
        return Get(header, index, out text);
    }

    // Token: 0x06000749 RID: 1865 RVA: 0x0003FCD8 File Offset: 0x0003DED8
    public static AmbChunk Get(AMS_FS header, int index, out string sPath)
    {
        sPath = null;
        byte[] array = null;
        int offset = -1;
    ...
    public static AmbChunk Search(AMS_AMB_HEADER header, string filename)
    {
        for (int i = 0; i < header.file_num; i++)
        {
            if (header.files[i] == filename)
            {
                return new AmbChunk(header.data, header.offsets[i], header.lengths[i], header);
            }
        }
    ...
    public static AmbChunk SearchEx(AMS_AMB_HEADER header, string exname)
    {
        AmbChunk result = null;
        for (int i = 0; i < header.file_num; i++)
        {
            ...
            {
                result = new AmbChunk(header.data, header.offsets[i], header.lengths[i], header);
                break;
            }
        }
    ...
    public byte[] SearchID(ref AMS_AMB_HEADER header, string file_id, byte[] top)
    {
        AppMain.mppAssertNotImpl();
        return null;
    }
